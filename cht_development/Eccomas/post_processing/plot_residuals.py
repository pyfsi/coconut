...
if reverse:
    ...
    common_path = '../Transient/fast/'
f = '/case_results.pickle'

if debug:
    case_paths = ['0.01' + f, '0.0001' + f]
    legend_entries = ['0.01', '0.0001']
else:
...
for case, residuals in zip(legend_entries, residual_list):
    avg_iterations = reduce(lambda count, l: count + len(l), residuals, 0)/len(residuals)
