...
class Transient_test():
    def calculate_DISPLACEMENT(self, x, y, z, n):
        """ Specify the displacement of a point on the interface relative to its original position"""
        ## Impose a sinusoidal variation of the radius for the tube running in time
        n_period=10
        R_orig = 0.005
        L = 0.05
        delta_R = R_orig / 10

        r_vec = np.array([0, y, z])
        r_vec_n = r_vec / np.linalg.norm(r_vec)

        dX = 0
        dY = delta_R * np.sin(2 * np.pi * ((x + L / 2.0) / L - n / n_period)) * r_vec_n[1]
        dZ = delta_R * np.sin(2 * np.pi * ((x + L / 2.0) / L - n / n_period)) * r_vec_n[2]
        disp = [dX,dY,dZ]
