...
for nt in range(2,nTS+1):
    ...
    os.system(f"sed -i 's/.*startTime.*/startTime {(nt-1)*TS};/' ./system/controlDict ") #Start from 0
    os.system(f"sed -i 's/.*endTime.*/endTime {nt*TS};/' ./system/controlDict  ")
    ...
    for p in range(0,nproc):
        ...
        f_disp_orig = f"./processor{p}/{(t-TS):.6f}/pointDisplacement_orig"
        f_disp = f"./processor{p}/{(t-TS):.6f}/pointDisplacement"
    ...
    os.system(f"mpirun -np {nproc} pimpleDyMFoam -parallel &>> log_remaining_TS")
