...
os.system(f"sed -i 's/.*startTime.*/startTime 0;/' ./system/controlDict ") #Start from 0
...
for i in range(0,n_points_b):
        ...
        X = orig_coords_b[i,0]
        dX = 0
        dY = delta_R * np.cos(2 * np.pi * ((X + L / 2.0) / L - t / T)) * r_vec_n[1]
        dZ = delta_R * np.cos(2 * np.pi * (X + L / 2.0) / L - t / T) * r_vec_n[2]
        disp_b[i,0] = dX
        disp_b[i, 1] = dY
        disp_b[i, 2] = dZ
...
os.system("decomposePar -fields &> log_decompose_fields")
