...
'''This python code is mainly aimed at testing whether the pointDisplacement file (if present) is read by OpenFoam when the solver is in a loop
currenlty with writeInterval set to 1'''
...
cmd = "mpirun -np 2 CoCoNuT_pimpleFoamTest -parallel &> log"
...
for p in range(0,nproc):
    ...
    f_disp_orig = f"./processor{p}/0.000010/pointDisplacement_orig"
    f_disp = f"./processor{p}/0.000010/pointDisplacement"
...
while not os.path.isfile('./continue_ready.coco'):
    time.sleep(1)
print("detected continue_ready.coco for time step 2")
os.system("rm continue_ready.coco")
