...
openfoam_process = subprocess.Popen(cmd, shell=True)#Popen= process open
...
for nt in range(2,nTS+1):
    ...
    os.system("decomposePar -fields &> log2_decompose_fields")
...
os.system("reconstructPar &> log_reconstruct")

#first step after restart
print("Launch restart")
mt=nt+1
print(f"Start TS {mt}")
t = (mt)*TS
...
os.system("decomposePar &> log_decompose")
...
cmd = f"mpirun -np {nproc} CoCoNuT_pimpleFoam -parallel &> log2_N_timesteps"
openfoam_process = subprocess.Popen(cmd, shell=True)#Popen= process open
...
for nt in range(mt+1,15):
    ...
    os.system("decomposePar -fields &> log3_decompose_fields")
    ...
    for p in range(0,nproc):
        ...
        os.system(f"cp {f_disp_orig} {f_disp}")dZ = delta_R * np.sin(2 * np.pi * (X + L / 2.0) / L - t / T) * r_vec_n[2]
        bool_start = 0
        bool_end = 0
        with open(f_disp_orig,"r") as fo:
            with open(f_disp,"w") as fd:
...
os.system(f"reconstructPar -newTimes  &> log3_reconstruct")
#plt.show()