...
class TestMapperInterpolator(unittest.TestCase):
    def test_bounding_box_1d(self):
        ...
        with self.assertWarns(Warning) as tmp:  # check that no warnings are thrown
            ...
            check_bounding_box(mp_a, mp_b, directions)
            ...
            mp_b = model.create_model_part('mp_b_3bis', *split(np.vstack((coords, np.array([[10, -10, 0]])))),
                                           np.arange(5))
            check_bounding_box(mp_a, mp_b, directions)
        ...
        self.assertWarns(Warning, check_bounding_box, *(mp_a, mp_b, directions))

        coords = np.vstack((coords, np.array([[0, 0, 1.25]])))
        mp_b = model.create_model_part('mp_b_5', *split(coords), np.arange(6))
        self.assertRaises(ValueError, check_bounding_box, *(mp_a, mp_b, directions))
    ...
    def test_bounding_box_2d(self):
        ...
        with self.assertWarns(Warning) as tmp:  # check that no warnings are thrown
            ...
            mp_b = model.create_model_part('mp_b_3bis', *split(np.vstack((coords, np.array([[0, -10, 0]])))),
                                           np.arange(5))
            check_bounding_box(mp_a, mp_b, directions)
