...
class TestSolverWrapperTubeStructureSolver(unittest.TestCase):
    ...
    def test_solver_wrapper_tube_structure_solver(self):
        parameter_file_name = os.path.join(os.path.dirname(__file__), 'test_tube_structure',
                                           'test_tube_structure_solver.json')
        ...
        if os.getcwd() == os.path.realpath(os.path.dirname(__file__)):
            parameters_solver['settings']['working_directory'] = 'test_tube_structure/CSM'
