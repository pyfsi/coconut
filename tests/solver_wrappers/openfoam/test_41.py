...
class TestSolverWrapperOpenFoam41(unittest.TestCase):
    ...
    def rm_time_directory(self, cores, time_step):

        if cores > 1:
            for core in range(cores):
                time_dir = os.path.join(self.folder_path, f'processor{core}/{(time_step*self.delta_t):.{self.t_prec}f}')
                rmtree(time_dir)
        else:
            time_dir = os.path.join(self.folder_path, f'{time_step*self.delta_t}:.{self.t_prec}f')
            rmtree(time_dir)
    ...
    def test_restart(self):
        ...
        self.par_solver['settings']['save_restart'] = -2
        solver = create_instance(self.par_solver)
        interface_input = solver.get_interface_input()
        ...
        #remove 3rd and 4th time steps
        self.rm_time_directory(cores=cores, time_step=3)
        self.rm_time_directory(cores=cores, time_step=4)
