...
class TestMapper_load_update(unittest.TestCase):
    ...
    def setUp(self):
        self.parameters = {'type': 'mappers.initial_load',
                           'settings':
                               {"initial_radius": 0.00074929,
                                "initial_depth": 0.0000235}
                           }
    ...
    def test_call(self):
        ...
        mapper = create_instance(self.parameters)
        mapper.initialize(model, mp_name_to, mp_name_from, forward=True)
        ...
        v_v_to_ref = np.zeros((self.mp_to.size,3))
        ...
        #                 pivot='tail', arrow_length_ratio=0.05, normalize=False, length=0.08, colors= 'b', linewidth=5)
        #
        #     for ax in [ax_s, ax_v]:
        #         ax.set_xlabel('x')
        #         ax.set_ylabel('y')
