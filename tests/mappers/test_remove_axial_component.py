...
class TestMapperPlasticDeform(unittest.TestCase):
    ...
    def test_initialize(self):
        ...
        #create MP_IN
        n_in = 10
        x0 = np.linspace(0,0.0035, n_in)
        r0 = np.full((n_in,),0.00075)
        z0= np.zeros(n_in)

        coords_init = np.column_stack((x0, r0, z0))

        model = data_structure.Model()
        model.create_model_part(mp_name_in, x0, r0, z0, np.arange(n_in))

        n_out_ref = n_in
        x_out_ref = coords_init[:,0]
        y_out_ref = coords_init[:,1]
        z_out_ref = coords_init[:,2]
    ...
    def test_call(self):
        ...
        parameters_from = [{'model_part': mp_name_from, 'variables': [var_v]}]
        ...
        # interface_to.set_variable_data(mp_name_from, var_s, v_s_from)
        ...
        parameters_to = [{'model_part': mp_name_to, 'variables': [var_v]}]
