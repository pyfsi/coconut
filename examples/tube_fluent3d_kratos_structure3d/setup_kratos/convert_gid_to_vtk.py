    ...
    def ReadResults(self):
        ...
        if not index == -1:
            ...
            for line in result_lines:
                self.displacement_results.append([float(line.split(' ')[1]), float(line.split(' ')[2]), float(line.split(' ')[3])])
    ...
    def WriteNodalResults(self):
        ...
        with open(self.output_vtk_file_name, 'a') as output_vtk_file:
            ...
            for node_variable in node_variables:
                output_vtk_file.write(f'{node_variable["variable_name"]} {node_variable["nr_components"]} {len(self.coordinates)}  float\n')
