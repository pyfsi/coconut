...
class TestSolverWrapperOpenFOAMExtend(unittest.TestCase):
    ...
    def read_face_centres(self, boundary_name, nfaces):
        filename_ccx = os.path.join("/lusers/temp/mathieu/PycharmProjects/coconut/examples/test_pressure_pertubation_structureFE41_coarse_1inner_iter/test_v41/wire/CSM",'0.00010','ccx')
        filename_ccy = os.path.join("/lusers/temp/mathieu/PycharmProjects/coconut/examples/test_pressure_pertubation_structureFE41_coarse_1inner_iter/test_v41/wire/CSM",'0.00010','ccy')
        filename_ccz = os.path.join("/lusers/temp/mathieu/PycharmProjects/coconut/examples/test_pressure_pertubation_structureFE41_coarse_1inner_iter/test_v41/wire/CSM",'0.00010','ccz')
    ...
    def test_displacement(self):
        ...
        for i in range(timesteps):
            ...
            for pr in pr_list:
                ...
                nfaces = mp.size
                filename_displacement = os.path.join("/lusers/temp/mathieu/PycharmProjects/coconut/examples/test_pressure_pertubation_structureFE41_coarse_1inner_iter/test_v41/wire/CSM",'0.00010','U')
            ...
            #     axs[2].legend(loc = 2)
            #
            plt.xlabel("position wire-die (m)")
            plt.ylabel("radial displacement (m)")
            plt.legend()
            plt.show()

            # print("displacement")
            difference = displacement_field[1]-displacement_field[0]
            plt.plot(x,difference)
            plt.xlabel("position wire-die (m)")
            plt.ylabel("difference radial displacement (m)")
            plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
            plt.show()
            print("cell_centre_diff")
            n =  cell_centres[1]-cell_centres[0]
