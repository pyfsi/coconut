...
shutil.copytree('setup_files/fluent3d_steady', cfd_dir)
cfd_env = tools.get_solver_env(cfd_solver, cfd_dir)
subprocess.check_call('./setup_fluent3d_steady.sh', shell=True, cwd=cfd_dir, env=cfd_env)

# create new CSM folder
shutil.copytree('setup_files/abaqus3d_steady', csm_dir)
csm_env = tools.get_solver_env(csm_solver, csm_dir)
subprocess.check_call('./setup_abaqus3d_steady.sh', shell=True, cwd=csm_dir, env=csm_env)
