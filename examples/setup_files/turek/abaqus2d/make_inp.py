...
step1 = beamModel.ImplicitDynamicsStep(name='Step-1', previous='Initial', timePeriod=delta_t, nlgeom=ON, maxNumInc=1, haftol=1, initialInc=delta_t, minInc=delta_t, maxInc=delta_t, amplitude=RAMP, noStop=OFF, nohaf=ON, initialConditions=OFF, timeIncrementationMethod=FIXED, application=MODERATE_DISSIPATION)
...
beamModel.SurfaceTraction(name = 'DistributedShear0', createStepName = 'Step-1', region = movingSurface0, magnitude = 1, traction = GENERAL, directionVector = ((0,0,0), (1,0,0)), distributionType = USER_DEFINED, follower = OFF)
beamModel.SurfaceTraction(name = 'DistributedShear1', createStepName = 'Step-1', region = movingSurface1, magnitude = 1, traction = GENERAL, directionVector = ((0,0,0), (1,0,0)), distributionType = USER_DEFINED, follower = OFF)
beamModel.SurfaceTraction(name = 'DistributedShear2', createStepName = 'Step-1', region = movingSurface2, magnitude = 1, traction = GENERAL, directionVector = ((0,0,0), (1,0,0)), distributionType = USER_DEFINED, follower = OFF)
if gravity > 0.0:
	beamModel.Gravity(name = 'Gravity', createStepName = 'Step-1', comp2 = -gravity)
beamModel.DisplacementBC(name = 'FixedLeftEnd', createStepName = 'Step-1', region = beamAssembly.sets['BEAMINSIDEFIXED'], u1 = 0, u2 = 0, ur3 = 0)
beamModel.FieldOutputRequest(createStepName='Step-1', frequency=LAST_INCREMENT, name='F-Output-1', variables=('U','COORD'))
