...
---:|:---:|---
`case_file`|string|Name of the case file. It must be present in the folder specified by `working_directory`.
...
<nobr>`hybrid_initialization`</nobr>|bool|If `true`, the hybrid initialization is used in Fluent before the first timestep. If `false`, the standard initialization is used, which requires that adequate reference values have been set in advance by the user in the Fluent case.
