...
Over the course of the restarted simulation, they will also be present in the woring directory since Abaqus needs to read these files in order to perform the restart. At the end of the simulation, they are removed from the working directory, but not from the *`vault`*.
