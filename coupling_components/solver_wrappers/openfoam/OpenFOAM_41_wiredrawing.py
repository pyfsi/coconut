    ...
    def __init__(self, parameters):
        ...
        for boundary in self.boundary_names:
            ...
            for i in np.arange(0, len(face_centres)):
                if "nonuniform" in fx_lines[index_x +3]:
                    x0[i] = float(fx_lines[i + 6 + index_x].split("\n")[0])
                else:
                    s = list(fx_lines[index_x + 3])
                    r = s[-3]
                    x0[i] = float(r)
                if "nonuniform" in fy_lines[index_y + 3]:
                    y0[i] = float(fy_lines[i + 6 + index_y].split("\n")[0])
                else:
                    s = list(fy_lines[index_y + 3])
                    r = s[-3]
                    y0[i] = float(r)
                if "nonuniform" in fz_lines[index_z + 3]:
                    z0[i] = float(fz_lines[i + 6 + index_z].split("\n")[0])
                else:
                    s = list(fz_lines[index_z + 3])
                    r = s[-3]
                    z0[i] = float(r)
