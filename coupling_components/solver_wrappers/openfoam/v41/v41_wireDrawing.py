...
class SolverWrapperOpenFOAM_41(Component):
        ...
        with open(controlDict_raw_name, 'r') as rawFile:
            with open(controlDict_name, 'w') as newFile:
                for line in rawFile:
                    ...
                    line = line.replace('|ADJUST_TIME|', str(self.adjust_time))
        ...
        with open(dynamicMeshDict_raw_name, 'r') as rawFile:
            with open(dynamicMeshDict_name, 'w') as newFile:
                for line in rawFile:
                    ...
                    line = line.replace('|INTERFACE_INPUT|',
                                        str(self.diffusivityX) + " " + str(self.diffusivityY) + " " + str(
                                            self.diffusivityZ))
                    newFile.write(line)
        rawFile.close()
        newFile.close()
    ...
    def Initialize(self):
        ...
        pointDisp_raw_name = os.path.join(os.path.realpath(os.path.dirname(__file__)), "pointDisplacement_wireDrawing")
        ...
        if not (os.path.isfile(pointDisp_name)):
            ...
            for boundary in self.boundary_names:
                ...
                fALines[index_A + 3] = '\t' '\t' ' ' "value" '\t' " uniform (" + str(self.displacementX) + " " + str(
                    self.displacementY) + " " + str(self.displacementZ) + ");" '\n'
        ...
        '''
        if self.cores > 1:
            os.system("cd " + self.working_directory + "; decomposePar -force -cellDist -time " + str(
                self.start_time) + " &> log.decomposePar;")
            for boundary in self.boundary_names:
                mp_output = self.model[boundary + "_output"]
                mp_output.sequence = []
                print(boundary + " mp_output.start_face")
                print(mp_output.start_face)
                print(boundary + " mp_output_nFaces")
    ...
    def write_node_input(self):
        ...
        for boundary in self.boundary_names:
            ...
            if self.iteration == 1:
                ...
                os.system(
                    "tail -n " + str(length_disp - (startNr + 1) - startToEndNr) + " " + disp_file + " > tempDisp3")
        ...
        if self.cores > 1:
            os.system(
                "cd " + self.working_directory + "; decomposePar -fields -cellDist -time " + self.prev_timestamp + " &> log.decomposePar;")
