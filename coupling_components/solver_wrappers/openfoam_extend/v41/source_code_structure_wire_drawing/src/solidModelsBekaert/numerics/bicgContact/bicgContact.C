...
#include "bicgContact.H"

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(bicgContact, 0);

    lduSolver::addsymMatrixConstructorToTable<bicgContact>
        addbicgContactSymMatrixConstructorToTable_;
}


// * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * * //

void Foam::bicgContact::Amul
(
    scalarField& Ax,
    const scalarField& x,
...
) const
{
    ...
    // Include additional coupling

    // Cells 70-79 coupled to cells 80-89
    ...
    for (int i = 73; i < 77; i++)
    {
        const scalar contrib = k*(x[i] - x[i + 10]);
        Ax[i] += contrib;
        Ax[i + 10] -= contrib;
    }
};
...
Foam::lduSolverPerformance Foam::bicgContact::solve
(
    scalarField& x,
    const scalarField& b,
    const direction cmpt
) const
{
    ...
    Amul(wA, x, coupleBouCoeffs_, interfaces_, cmpt);
    ...
    if (!stop(solverPerf))
    {
        ...
        do
        {
            ...
            Amul(wA, pA, coupleBouCoeffs_, interfaces_, cmpt);
