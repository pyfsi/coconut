...
void TLMaterialInterface::correct(fvVectorMatrix& DEqn)
{
    ...
    Switch nonLinear = false;
    Switch enforceLinear = true;
    Switch thermalStress =
        Switch
        (
            mesh().objectRegistry::foundObject<thermalModel>
            (
                "thermalProperties"
            )
        );
    ...
    if (thermalStress)
    {
        ...
        threeKalphaPtr =
            const_cast<volScalarField*>
            (
                &mesh().lookupObject<volScalarField>("((threeK*rho)*alpha)")
            );
    }
...
//             tensor ngbSGradD = ((I-ngbN*ngbN)&gradDI[curNeighbour]);
            ...
            if (thermalStress)
            {
                ...
                scalar ownThreeKalpha = threeKalpha.internalField()[curOwner];
                scalar ngbThreeKalpha =
                    threeKalpha.internalField()[curNeighbour];
                ...
                ownThermalTraction = -ownThreeKalpha*interfaceDT*ownN;
                ngbThermalTraction = -ngbThreeKalpha*interfaceDT*ownN;
            }
            ...
            // philipc: thermal stress produces only normal traction
        ...
        else
        {
            ...
            if (thermalStress)
            {
                ...
                scalar ownThreeKalpha =
                    threeKalpha.internalField()[curPatchCells[curPatchFace]];
                scalar ngbThreeKalpha =
                    threeKalpha.boundaryField()[curPatch][curPatchFace];
                scalar interfaceDT =
...
void TLMaterialInterface::correct(surfaceVectorField& trac)
{
    forAll(faces(), faceI)
    {
        ...
        if (curFace < mesh().nInternalFaces())
        {
            //Info<<"Correcting traction on face " << curFace
            //    << " to " << traction()[faceI] << endl;
