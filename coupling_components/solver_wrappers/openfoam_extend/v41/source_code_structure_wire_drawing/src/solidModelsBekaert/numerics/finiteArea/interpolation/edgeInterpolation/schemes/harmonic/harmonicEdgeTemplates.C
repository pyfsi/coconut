...
template<class Type>
Foam::tmp<Foam::edgeScalarField> Foam::harmonicEdgeInterpolation<Type>::weights
(
...
) const
{
    ...
    tmp<edgeScalarField> tw
    (
        new edgeScalarField
        (
            IOobject
            (
                "harmonicEdgeWeightingFactors" + phi.name(),
    ...
    const edgeScalarField& deltaCoeffs = this->mesh().deltaCoeffs();
    const edgeScalarField& weights = this->mesh().weights();

    const magLongDeltaFa& mld = magLongDeltaFa::New(this->mesh());
    const edgeScalarField& longDelta = mld.magDelta();

    edgeScalarField& w = tw();
    ...
    {
        scalarField& wp = w.boundaryField()[pi];

        const faPatchField<Type>& pPhi = phi.boundaryField()[pi];
...
template<class Type>
Foam::tmp<Foam::scalarField> Foam::harmonicEdgeInterpolation<Type>::weights
(
