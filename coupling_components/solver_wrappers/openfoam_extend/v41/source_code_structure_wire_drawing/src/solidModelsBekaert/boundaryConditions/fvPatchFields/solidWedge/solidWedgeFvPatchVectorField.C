...
tmp<Field<vector> > solidWedgeFvPatchVectorField::snGrad() const
{
    ...
    )
    {
        ...
        // We can then calculate snGrad by getting the difference between projU
        // and transformed projU (i.e. across the wedge), and then dividing by
        // the magnitude of the distance between them.
        ...
        const scalarField d = ((patch().Cn() - patchC) & centreN)/(nHat & centreN);
        const vectorField projC = d*nHat + patchC;
        ...
        const Field<vector> projU = pif + (k & gradU.patchInternalField());
