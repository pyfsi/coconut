...
:
    solidDirectionMixedFvPatchVectorField(p, iF),
    totRefValue_(p.size(), vector::zero),
    totalFieldName_(dict.lookupOrDefault<word>("totalFieldName", "U")),
    cohesiveLawPtr_
    (
...
label cohesiveZoneIncrementalFvPatchVectorField::updateCrack()
{
    ...
    vectorField curTangentialTraction = ((I- sqr(n)) & (n & curSigma));

    // Current normal traction increment
    scalarField curNormalTractionIncrement = (n &(n & DSigma));

    // Philipc: new traction to be set
    vectorField newTraction(oldTraction.size(), vector::zero);
    ...
    forAll(curNormalTraction, faceI)
    {
        if
        (
            (magSqr(this->valueFraction()[faceI]) > (1.0 - SMALL))
...
//     bool breakOnlyOneFace = true;
    ...
    forAll(facesToBreak, fI)
    {
        ...
        Info<< "Switching valueFraction to zero for face " << faceI << endl;
...
//           + curTangentialTraction[faceI];
        ...
        newTraction[faceI] = cohesiveTractionIncrement + oldTraction[faceI];
    ...
    //     if (stress.mechanical().plasticityActive())
    //     {
    //         this->refGrad()[faceI] +=
    //             2*mu[faceI]
    //            *(
    ...
    // Philipc: set traction
    this->refGrad() =
        tractionBoundaryGradient().snGrad
        (
            newTraction,                // surface traction
            scalarField(newTraction.size(), 0.0), // surface pressure
            DDName,                     // working field name
            "U",                        // total field name
            patch(),                    // polyPatch
            true                        // incremental
        );
...
void cohesiveZoneIncrementalFvPatchVectorField::updateCoeffs()
{
    ...
    if (max(mag(separationDistance_)) > 1.0)
    {
        FatalError
...
//                   - oldRelaxedTraction[faceI];
    ...
    this->refGrad() =
        ...
        (
            ...
            scalarField(newTraction.size(), 0.0), // surface pressure
            DDName,                     // working field name
            totalFieldName_,            // total field name
            patch(),                    // polyPatch
            true                        // incremental
        );
