...
bool Foam::crackerFvMesh::update()
{
    ...
    // Clear out the law demand driven data
    lawPtr_().clearOut();

    // Get faces to break from the law
    const labelList& facesToBreak = lawPtr_().facesToBreak();
    const boolList& facesToBreakFlip = lawPtr_().facesToBreakFlip();
    const labelList& coupledFacesToBreak = lawPtr_().coupledFacesToBreak();
    ...
    if (nFacesToBreak || nCoupledFacesToBreak)
    {
        ...
        // Scale tractions on newly broken faces: this is not needed as the
        // traction field in solidCohesive will automatically recalculate it
        //Warning
        //    << "cohesive tractions not scaled" << endl;
        // scaleCohesiveTractions
        // (
        //     faceMap,
        //     facesToBreak,
        //     coupledFacesToBreak,
