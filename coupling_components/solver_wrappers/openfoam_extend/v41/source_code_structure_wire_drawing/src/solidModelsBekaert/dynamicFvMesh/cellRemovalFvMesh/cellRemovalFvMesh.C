...
:
    ...
    meshModPtr_(NULL),
    openPatchID_(boundaryMesh().findPatchID(dict_.lookup("openPatch"))),
    cellRemover_(*this, true),
    lawPtr_(cellRemovalLaw::New("law", *this, dict_.subDict("law")))
{
    // Check openPatch was found
    if (openPatchID_ == -1)
    {
        FatalErrorIn("Foam::cellRemovalFvMesh::cellRemovalFvMesh")
            << "openPatch not found!" << abort(FatalError);
    }
}
...
bool Foam::cellRemovalFvMesh::update()
{
    ...
    const labelList cellsToRemove = lawPtr_().cellsToRemove();

    label nCellsToRemove = cellsToRemove.size();
    reduce(nCellsToRemove, maxOp<label>());

    if (nCellsToRemove)
    {
        // Exposed faces will be inserted into the open patch

        Info<< nl << "There are " << cellsToRemove.size()
            << " cells to be removed" << endl;

        // Find faces that will be exposed
        // These faces will become boundary faces in openPatch
        ...
        (
            cellsToRemove,
            facesToExpose,
            labelList(facesToExpose.size(), openPatchID_),
            meshModifier()
        );
        ...
        if (removeDeadCells_)
        {
            ...
            const label deadCellLimit = 0.01*nCells();
        ...
        //     (
        //         lookupObject<volVectorField>("DU")
        //     );
