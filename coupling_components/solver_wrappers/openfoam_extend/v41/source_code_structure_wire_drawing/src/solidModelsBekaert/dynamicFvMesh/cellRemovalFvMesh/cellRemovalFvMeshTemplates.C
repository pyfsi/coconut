...
) const
{
    ...
    )
    {
        ...
        Field<Type>& fieldI = field.internalField();
        ...
        forAll(exposedFaces, fI)
        {
            ...
            if
            (
                field.boundaryField()[patchID].type()
             == fvPatchField<Type>::calculatedType()
             ) //fixed, by Haotian
            {
                field.boundaryField()[patchID][newLocalFaceID] =
                    fieldI[faceCellID];
            }
            ...
            if (surfaceFieldExists)
            {
                ...
                surfaceField.boundaryField()[patchID][newLocalFaceID] =
                    fieldI[faceCellID];
            }
        }
    }
