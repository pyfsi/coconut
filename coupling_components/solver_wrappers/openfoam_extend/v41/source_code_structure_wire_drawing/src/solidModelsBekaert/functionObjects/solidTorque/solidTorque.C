...
//#include "freeRollerFvPatchVectorField.H"
    ...
    historyFilePtr_(NULL)
{
    ...
    // Check the patch is a roller as opposed to a freeRoller
    if
    (
        mesh.lookupObject<volVectorField>("DU").boundaryField()
        [
            historyPatchID_
        ].type()
     != rollerFvPatchVectorField::typeName
    )
    {
        patchFound_ = false;

        InfoIn(this->name() + " function object constructor")
            << "history patch " << historyPatchName << " is a freeRoller so "
            << "history data will not be printed"
            << endl;
    }
        ...
        if (Pstream::master())
        {
            ...
            historyFilePtr_.reset
                (
                    new OFstream
                    (
                        historyDir/"solidTorque" + historyPatchName + ".dat"
                    )
                );
