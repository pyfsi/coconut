...
namespace Foam
{
    defineTypeNameAndDebug(solidStress, 0);
...
bool Foam::solidStress::writeData()
{
    if (patchFound_)
    {
        const fvMesh& mesh =
        ...
        if (mesh.foundObject<volSymmTensorField>(stressName_))
        {
            ...
            // Patch force
            symmTensor avStress = gAverage(sigma);
    ...
    historyFilePtr_(NULL)
{
    ...
    else
    {
        WarningIn(this->name() + " function object constructor")
            << "solidStress: historyPatch not specified" << endl;
    }
        ...
        if (Pstream::master())
        {
            ...
            historyFilePtr_.reset
                (
                    new OFstream
                    (
                        historyDir/"solidStress"+historyPatchName+".dat"
                    )
                );
...
#if FOAMEXTEND > 40
bool Foam::solidStress::execute(const bool forceWrite)
#else
bool Foam::solidStress::execute()
#endif
{
    return writeData();
}
...
bool Foam::solidStress::read(const dictionary& dict)
{
    return true;
}
