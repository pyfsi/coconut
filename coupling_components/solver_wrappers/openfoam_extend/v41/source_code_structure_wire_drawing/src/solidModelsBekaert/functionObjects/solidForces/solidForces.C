        ...
        if (mesh.foundObject<volSymmTensorField>(stressName_))
        {
            ...
            if (nonLinearTL_)
            {
                ...
                force =
                    gSum
                    (
                        (Finv.T() & mesh.Sf().boundaryField()[historyPatchID_])
                        & sigma
                    );
            }
            else
            {
                force =
                    gSum(mesh.Sf().boundaryField()[historyPatchID_] & sigma);

                normalForce =
                    gSum
                    (
                        mesh.boundary()[historyPatchID_].nf()
    ...
    historyFilePtr_(NULL)
{
    ...
    if (nonLinearTL_)
    {
        Info<< "nonLinear total Lagrangian is true" << endl;
    }
