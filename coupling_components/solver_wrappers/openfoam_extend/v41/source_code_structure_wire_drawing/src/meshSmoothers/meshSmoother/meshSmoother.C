...
)
{
    ...
    PtrList<pointField> patchPointNormals(mesh.boundaryMesh().size());
    forAll(mesh.boundaryMesh(), patchI)
    {
        patchPointNormals.set
        (
            patchI,
            new pointField(mesh.boundaryMesh()[patchI].nPoints(), vector::zero)
        );
    }
    pointNormalsConsistent(patchPointNormals, mesh);
    ...
    forAll(mesh.boundaryMesh(), patchI)
    {
        ...
        {
            const vectorField& pointNormals = patchPointNormals[patchI];
    ...
    correctFixedPatchesMotion(pointMotionD);
    correctFlatPatchesMotion(pointMotionD);
}


void meshSmoother::correctFixedPatchesMotion
(
    pointVectorField& pointMotionD
) const
{
...
void meshSmoother::correctFlatPatchesMotion
(
    pointVectorField& pointMotionD
) const
{
...
void meshSmoother::setNormalDisplacementToZeroOnSymmetryPlanes
(
    pointVectorField& pointMotionD
) const
{
    const polyMesh& mesh = pointMotionD.mesh().mesh();
    pointField& pointDUI = pointMotionD.internalField();
    ...
    forAll(mesh.boundaryMesh(), patchI)
    {
        if (isA<symmetryPolyPatch>(mesh.boundaryMesh()[patchI]))
        {
            ...
            const vector avgN = gAverage(patchPointNormals[patchI]);
...
{
    // Check the advection scheme is not "Gauss linear" as this is unstable
    Istream& is = mesh.schemesDict().divScheme("advectFields");
    if (word(is) == "Gauss")
    {
        if (word(is) == "linear")
        {
            FatalErrorIn(type() + "::" + type() + "()")
                << "'advectFields' divScheme should not be 'Gauss linear' as it "
                << "is unstable" << nl
                << "Add, for example, 'advectFields    Gauss upwind;' to "
                << "divSchemes" << abort(FatalError);
        }
    }
}
...
)
{
    ...
    ignoreFields[6] = "curMaterial0";
    ignoreFields[7] = "curMaterial1";
    ignoreFields[8] = "curMaterial2";
    ignoreFields[9] = "curMaterial3";
    ignoreFields[10] = "curMaterial4";
    ignoreFields[11] = "curMaterial5";
    ignoreFields[12] = "curMaterial6";
    ignoreFields[13] = "curMaterial7";
    ignoreFields[14] = "curMaterial8";
    ignoreFields[15] = "curMaterial9";
    ignoreFields[16] = "curMaterial10";
    ...
    (
        sweptVolField, volOldField, volNewField,
        //areasOld, areasNew, sweptAreas,
        ignoreFields
    );
