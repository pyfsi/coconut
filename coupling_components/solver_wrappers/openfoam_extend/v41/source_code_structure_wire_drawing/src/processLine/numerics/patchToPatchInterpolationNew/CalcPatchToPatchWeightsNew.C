...
void PatchToPatchInterpolationNew<FromPatch, ToPatch>::calcPointAddressing() const
{
    ...
    forAll (pointAddressing, pointI)
    {
        ...
        // PC: you cannot ask for the hitFace if it does not exist!
        // Skip these faces!
        if (!proj[pointI].hit())
        {
            continue;
...
template<class FromPatch, class ToPatch>
void PatchToPatchInterpolationNew<FromPatch, ToPatch>::calcFaceAddressing() const
{
