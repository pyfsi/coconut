...
template<class Type>
tmp<Field<Type> >
PatchToPatchInterpolationNew<FromPatch, ToPatch>::pointInterpolate
(
    const Field<Type>& pf
) const
{
    if (pf.size() != fromPatch_.nPoints())
    {
        FatalErrorIn
        (
            "PatchToPatchInterpolationNew::pointInterpolate"
            "(const Field<Type> pf)"
...
template<class Type>
tmp<Field<Type> >
PatchToPatchInterpolationNew<FromPatch, ToPatch>::faceInterpolate
(
    const Field<Type>& ff
) const
{
    if (ff.size() != fromPatch_.size())
    {
        FatalErrorIn
        (
            "PatchToPatchInterpolationNew::faceInterpolate"
            "(const Field<Type> ff)"
        )   << "given field does not correspond to patch. Patch size: "
