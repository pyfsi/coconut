...
void addPatchFields(fvMesh& mesh, const word& patchFieldType)
{
    ...
    (
        mesh.objectRegistry::lookupClass< const GeoField>()
    );
...
)
{
    ...
    // PhilipC 31/Jan/2017
    // Remove cellZones with size zero

     // Copy cell zones that have a non-zero size

    cellZoneMesh& cz = newMesh.cellZones();

    List<cellZone*> newCz(cz.size());
    ...
    forAll(cz, zoneI)
    {
        if (cz[zoneI].size() != 0)
        {
            Info<< "Adding cellZone " << cz[zoneI].name() << endl;

            // Copy non-zero sized cellZones to new list of cellZones
            newCz[nNewCZs] =
                new cellZone
                (
                    cz[zoneI].name(),
        ...
        {
            Info<< "Ignoring cellZone " << cz[zoneI].name() << " with size zero"
                << endl;
        }
    }

    newCz.resize(nNewCZs);

    // Clear the old cell zones
    cz.clear();
    cz.resize(nNewCZs);
    ...
    // Force cell zones to be written
    cz.writeOpt() = IOobject::AUTO_WRITE;
...
#   include "createMesh.H"
    ...
    else if (makeCellZones)
    {
        ...
        {
            Info<< " TEST: " << mesh.db().filePath() << endl;
