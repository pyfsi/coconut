...
Foam::processorMeshesReconstructor::reconstructMesh(const Time& db)
{
    ...
    // PhilipC 30-06-17: reconstuct cellZones

    const int nCellZones = meshes_[0].cellZones().size();

    if (nCellZones > 0)
    {
        Info<< nl << "Reconstructing " << nCellZones << " cellZones: " << nl;

        globalMesh.cellZones().setSize(nCellZones);

        for (int cellZoneI = 0; cellZoneI < nCellZones; cellZoneI++)
        {
            // Count the number of cells in cellZoneI

            int nCellsInCellZoneI = 0;

            forAll(meshes_, procMeshI)
            {
                if (meshes_[procMeshI].cellZones().size() != nCellZones)
                {
                    FatalError
                        << "Processor 0 has a different number of cellZones to "
                        << "processor " << procMeshI << abort(FatalError);
                }

                nCellsInCellZoneI +=
                    meshes_[procMeshI].cellZones()[cellZoneI].size();
            }

            // Create the global (reconstructed) cellZone addressing

            labelList globalCellZoneI(nCellsInCellZoneI, -1);

            label index = 0;

            forAll(meshes_, procMeshI)
            {
                const labelList& cpa = cellProcAddressing_[procMeshI];
                const labelList& cz = meshes_[procMeshI].cellZones()[cellZoneI];

                forAll(cz, cI)
                {
                    const label procCellID = cz[cI];

                    globalCellZoneI[index] = cpa[procCellID];

                    index++;
                }
            }

            const word& cellZoneName = meshes_[0].cellZones()[cellZoneI].name();

            Info<< "    " << cellZoneName << endl;

            globalMesh.cellZones().set
            (
                cellZoneI,
                new cellZone
                (
                    cellZoneName,             // name
                    globalCellZoneI,          // addressing
                    cellZoneI,                // index
                    globalMesh.cellZones()    // cellZoneMesh
                )
            );

            globalMesh.cellZones().writeOpt() = IOobject::AUTO_WRITE;
        }
    }
    Info<< nl;
