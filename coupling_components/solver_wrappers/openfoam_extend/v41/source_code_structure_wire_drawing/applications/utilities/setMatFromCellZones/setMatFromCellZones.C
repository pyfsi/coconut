...
Description
    ...
    This is works well for a multi-zone mesh created in snappyHexMesh, or
    any other meshing software that generates cellZones.
...
#   include "createMesh.H"
    ...
    forAll(cellZones, zonei)
    {
        Info<< "    Cell zone " << cellZones[zonei].name()
            ...
            << " set to material index " << zonei
            << endl;
        nCzCells += cellZones[zonei].size();
    }
    ...
    {
        Warning
            << "There are cells which are not in a cellZone"
            ", their material field is set to "
            << cellZones.size() << endl;
    }

    Info << "Writing materials field\n" << endl;
    materials.write();
