    ...
    forAll(boundaries, patchI)
    {
        if( boundaries[patchI].patchType() == "symmetryPlane" )
        {
            ...
            for(label faceI=start;faceI<end;++faceI)
            {
                ...
                cs.first += help::faceCentre(points, f);
                ns.first += help::faceAreaVector(points, f);
            }
...
void symmetryPlaneOptimisation::optimizeSymmetryPlanes()
{
    ...
    forAll(pointInPlane, pointI)
    {
        ...
        if( nPlanes > 3 )
        {
            Warning << "Point " << pointI << " is in more than three symmetry"
        ...
        if( nPlanes == 1 )
        {
            ...
            meshModifier.movePoint(pointI, it->second.nearestPoint(p));
        }
        else if( nPlanes == 2 )
        {
            ...
            meshModifier.movePoint(pointI, il.refPoint() + lProj * n);
        }
        else if( nPlanes == 3 )
        {
            ...
            meshModifier.movePoint(pointI, pl0.planePlaneIntersect(pl1, pl2));
        }
    }
    ...
    if( badFaces.size() )
    {
        Warning << "Bad quality or inverted faces found in the mesh" << endl;
