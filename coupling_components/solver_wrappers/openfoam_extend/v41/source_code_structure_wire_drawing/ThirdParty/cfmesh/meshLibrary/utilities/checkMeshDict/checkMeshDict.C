...
void checkMeshDict::checkAutomaticRefinement() const
{
    if( meshDict_.found("automaticRefinement") )
    {
        ...
        {
            readScalar(autoRefDict.lookup("minCellSize"));
        }
        ...
        //- check if the refinement based on edge length shall be used globally
        ...
        if( autoRefDict.found("localRefinement") )
        {
            ...
            forAll(objectNames, objI)
            {
                ...
                if( dict.found("numCellsOverFeatureSize") )
                {
                    readLabel(dict.lookup("numCellsOverFeatureSize"));
                }

                //- check if curvature shall be used
                if( dict.found("curvatureRefinement") )
                {
                    readBool(dict.lookup("curvatureRefinement"));
                }

                //- check if edge length shall be used
                ...
                //- sticky distance applicable to local object
                if( autoRefDict.found("stickyDistance") )
                {
                    readScalar(autoRefDict.lookup("stickyDistance"));
                }
...
void checkMeshDict::checkBoundaryLayers() const
{
    if( meshDict_.found("boundaryLayers") )
    {
        ...
        if( bndLayers.found("skipRefinementOfPoorQualityLayerCells") )
        {
            readBool
            (
                bndLayers.lookup("skipRefinementOfPoorQualityLayerCells")
            );

            //- additional controls of the procedure
            if( bndLayers.found("maxOrthogonalityDeviationAngle" ) )
            {
                readScalar
                (
                    bndLayers.lookup("maxOrthogonalityDeviationAngle")
                );
            }

            //- min thickness to flatness deviation height
            if
            (
                bndLayers.found
                (
                    "minThicknessToFlatnessDeviationHeightRatio"
                )
            )
            {
                readScalar
        ...
        //- check for existence of topological settings
        if( bndLayers.found("inletOutletPatches") )
        {
            //- check if it is set as a dictionary
            if( !bndLayers.isDict("inletOutletPatches") )
                FatalErrorIn
                (
                    "void checkMeshDict::checkBoundaryLayers() const"
                ) << "inletOutletPatches must be a dictionary"
                  << exit(FatalError);
            ...
            forAll(patchNames, i)
            {
                ...
                if( pDict.found("generateLayer") )
                   readBool(pDict.lookup("generateLayer"));
            }
        }

        //- check for existence of symmetryPlaneLayerTopology
        if( bndLayers.found("symmetryPlaneLayerTopology") )
        {
            ...
            if( bndLayers.found("generateLayersAtSymmetryPlanes") )
                readBool(bndLayers.lookup("generateLayersAtSymmetryPlanes"));
        }
    }
}
...
void checkMeshDict::updateAutomaticRefinement
(
    const std::map<word, wordList>& patchesFromPatch
)
{
    if( meshDict_.found("automaticRefinement") )
    {
        ...
        if( autoRefDict.found("localRefinement") )
        {
            ...
            forAll(objectNames, objI)
            {
                ...
                std::map<word, wordList>::const_iterator it =
                    patchesFromPatch.find(objectNames[objI]);

                if( it != patchesFromPatch.end() )
                {
                    const wordList& newPatches = it->second;

                    dictionary copy = dict;

                    forAll(newPatches, i)
                        localDict.add(newPatches[i], copy, true);

                    //- remove the original
                    localDict.remove(objectNames[objI]);
                }
            }
        }
    }
...
)
{
    if( meshDict_.isDict("boundaryLayers") )
    {
        ...
        //- check for existence of topological settings
        if( bndLayersDict.found("inletOutletPatches") )
        {
            dictionary& inletOutletDict =
                bndLayersDict.subDict("inletOutletPatches");
            ...
            forAll(patchNames, i)
            {
                const word pName = patchNames[i];

                const dictionary dict = inletOutletDict.subDict(pName);
                ...
                if( it != patchesFromPatch.end() )
                {
                    ...
                    forAll(newNames, i)
                    {
                        inletOutletDict.add(newNames[i], dict);
                    }

                    inletOutletDict.remove(pName);
                }
            }
        }
    }
...
)
{
    ...
    updateAutomaticRefinement(patchesFromPatch);
