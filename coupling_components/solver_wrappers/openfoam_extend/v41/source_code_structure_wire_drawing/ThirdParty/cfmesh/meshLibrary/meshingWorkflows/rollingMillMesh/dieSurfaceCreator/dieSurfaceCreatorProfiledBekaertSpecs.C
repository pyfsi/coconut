...
#include "dieSurfaceCreatorProfiledBekaertSpecs.H"
...
defineTypeNameAndDebug(dieSurfaceCreatorProfiledBekaertSpecs, 0);
addToRunTimeSelectionTable
(
    dieSurfaceCreator,
    dieSurfaceCreatorProfiledBekaertSpecs,
    dictionary
);

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

void dieSurfaceCreatorProfiledBekaertSpecs::createLine
(
...
void dieSurfaceCreatorProfiledBekaertSpecs::createArc
(
    const point& firstPoint,
    const point& secondPoint,
    const point& centre,
...
void dieSurfaceCreatorProfiledBekaertSpecs::createTriangulatedSurface
(
    const scalar H,
    const scalar W,
    const scalar Con,
...
//    const scalar dy_6_7 = 0;
    ...
    # ifdef DEBUGDie
    surfPtr_->writeSurface("profiledBekaertSpecsDie.stl");
    Info << "Profiled BekaertSpecs die written" << endl;
    ::exit(0);
    # endif
}
...
dieSurfaceCreatorProfiledBekaertSpecs::dieSurfaceCreatorProfiledBekaertSpecs
(
    ...
    outletDiameter_(0.0)
{
    ...
    //- entrance bell chamfer length angle
    scalar CRA;
    ...
    //- TODO: calculate from profile data
    scalar EBR;
    ...
    //- reduction cone angle is calculated from the cross-section profiles
    scalar RCA(VSMALL);

    //- reduction cone length in x-dir
    scalar RCL;
    if( !dict.readIfPresent("RC_L", RCL) )
    {
         FatalError << "RC_L is not present in dictionary " << dict
            << exit(FatalError);
    }

    //- rounding radius between reduction cone and bearing
    scalar RCR;
    ...
    //- read the profile of the reduction cone cross-section
    scalar RC_radius(-1.0);
    fileName reductionConeProfileName;
    if( dict.readIfPresent("reductionConeDiameter", RC_radius) )
    {
        std::shared_ptr<triSurf> sPtr = createCircularProfile(2.0 * RC_radius);

        //- append cross-section to the list
    ...
    (
        dict.readIfPresent("reductionConeProfile", reductionConeProfileName)
    )
    {
        dieCrossSectionSurfaceCreator sectionCreator
        (
            reductionConeProfileName,
            geometryTol_
        );

        std::shared_ptr<triSurf> surfPtr =
            std::make_shared<triSurf>(sectionCreator.surface());
        ...
        )
        {
            ...
            (
                reductionConeProfileName.lessExt()+"_invalid.fms"
            );

            FatalError << "Reduction cone profile " << reductionConeProfileName
                << " is not valid. "
                << "It is written to "
                << reductionConeProfileName.lessExt()+"_invalid.fms"
                << exit(FatalError);
        ...
        if( maxRadius >= W )
        {
            ...
            ) << "Maximum diameter of a profile given in "
              << reductionConeProfileName
              << " is greater than the outer radius of a die "
              << exit(FatalError);
        }

        Info << "Reduction cone radius: min: " << minRadius
             << " max: " << maxRadius << endl;

        RC_radius = 0.5 * (maxRadius + minRadius);
    }
    else
    {
        FatalError << "reductionConeProfile or reductionConeDiameter "
            << " are not specified. Cannot continue." << exit(FatalError);
    }
        ...
        forAll(names, i)
        {
            if( profilesDict.isDict(names[i]) )
            {
                ...
                )
                {
                    surfPtr->writeSurface
                    (
                        fName.lessExt()+"_invalid.fms"
                    );

                    FatalError << "Intermediate profile " << fName
                        << " is not valid. "
                        << "It is written to "
                        << fName.lessExt()+"_invalid.fms"
                        << exit(FatalError);
    ...
    //- read the profile at the bearing cross-section
    fileName bearingProfileName;
    if( !dict.readIfPresent("bearingProfile", bearingProfileName) )
    {
        FatalError << "bearingProfile not found in dictionary "
    ...
    else
    {
        ...
        )
        {
            surfPtr->writeSurface
            (
                bearingProfileName.lessExt()+"_invalid.fms"
            );

            FatalError << "Bearing profile " << bearingProfileName
                << " is not valid. "
                << "It is written to "
                << bearingProfileName.lessExt()+"_invalid.fms"
                << exit(FatalError);
        ...
        if( maxRadius >= W )
        {
            ...
            (
                ...
                "const rollingMillPatchNamesHandler&,"
                " const dictionary&, const scalar tol)"
            ) << "Maximum diameter of a profile given in " << bearingProfileName
              << " is greater than the outer radius of a die "
              << exit(FatalError);
        }

        Info << "Bearing radius: min: " << minRadius
             << " max: " << maxRadius << endl;

        D = (maxRadius + minRadius);

        //- calculate reduction cone angle from
        RCA = atan((RC_radius-0.5*D) / (RCL + VSMALL)) * 180.0 / M_PI;

        Info << "Average bearing diameter " << D << endl;
        Info << "Average reduction cone angle " << RCA << endl;
    }
    ...
    //- sort cross-section profiles in ascending order
    bool changed;
    do
    {
        changed = false;
    ...
    # ifdef DEBUGDie
    surfPtr_->writeSurface("dieAxialProfile.stl");
    forAll(crossSections_, i)
    {
        triSurf copy(*crossSections_[i].second);
        triSurfModifier sMod(copy);
        forAll(sMod.pointsAccess(), pI)
            sMod.pointsAccess()[pI].x() += crossSections_[i].first;

        copy.writeSurface("crossSection"+help::labelToText(i)+".stl");
    }
    # endif
}
...
dieSurfaceCreatorProfiledBekaertSpecs::dieSurfaceCreatorProfiledBekaertSpecs
(
    const dieSurfaceCreatorProfiledBekaertSpecs& die
)
:
    dieSurfaceCreator
    (
...
scalar dieSurfaceCreatorProfiledBekaertSpecs::inletDiameter() const
{
    return inletDiameter_;
}

scalar dieSurfaceCreatorProfiledBekaertSpecs::outletDiameter() const
{
    return outletDiameter_;
}

scalar dieSurfaceCreatorProfiledBekaertSpecs::outerDiameter() const
{
