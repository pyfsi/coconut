...
bool checkIrregularSurfaceConnections::checkFacesWithOppositeNormals
(
...
)
{
    if( mesh_.returnTime().foundObject<IOdictionary>("meshDict") )
    {
        ...
        if( meshDict.found("nonManifoldMeshing") )
        {
            const bool nonManifold =
                readBool(meshDict.lookup("nonManifoldMeshing"));
    ...
    Info << "Checking for faces with opposite normals" << endl;
    ...
    std::set<label> problematicPoints;

    vectorField faceNormals(bFaces.size());
    ...
    # endif
    {
        ...
        forAll(bFaces, bfI)
        {
            ...
            faceNormals[bfI] /= (mag(faceNormals[bfI]) + VSMALL);
        }
        ...
        forAll(bPoints, bpI)
        {
            ...
            for(label i=0;i<pFaces.sizeOfRow(bpI);++i)
            {
                const vector& n = faceNormals[pFaces(bpI, i)];

                for(label j=i+1;j<pFaces.sizeOfRow(bpI);++j)
                {
                    if( (n & faceNormals[pFaces(bpI, j)]) < -0.99 )
                    {
                        //- found a point connected to two faces with opposite
                        //- normal directions
                        # ifdef USE_OMP
                        # pragma omp critical(facesWithOppositeNormals)
                        # endif
                        {
                            badVertices.insert(bpI);
                            problematicPoints.insert(bpI);
                        }

                        //- mark cells for removal
                        removeCell[faceOwners[pFaces(bpI, i)]] = true;
                        removeCell[faceOwners[pFaces(bpI, j)]] = true;
                    }
                }
            }
        }
    ...
    if( Pstream::parRun() )
    {
        ...
        //- send the normal directions to all neighbouring processors
        forAllConstIter(Map<label>, globalToLocal, it)
        {
            const label bpI = it();
            ...
            forAllRow(bpAtProcs, bpI, i)
            {
                ...
                forAllRow(pFaces, bpI, pfI)
                {
                    ...
                    dts.append(labelledPoint(it.key(), faceNormals[bfI]));
                }
            }
        }
        ...
        //- find the faces with opposite normal direction
        # ifdef USE_OMP
        ...
        forAll(receivedData, i)
        {
            ...
            forAllRow(pFaces, bpI, pfI)
            {
                ...
                if( (faceNormals[bfI] & receivedData[i].coordinates()) < -0.99 )
                {
                    //- these two faces have opposite normal direction
                    # ifdef USE_OMP
                    # pragma omp critical(facesWithOppositeNormals)
                    # endif
                    {
                    ...
                    //- mark owner cell for removal
                    removeCell[faceOwners[bfI]] = true;
                }
            }
        }
    }

    label nBadVertices = problematicPoints.size();
    reduce(nBadVertices, sumOp<label>());

    Info << "Found " << nBadVertices << " problematic vertices" << endl;
    Info << "Finished checking for faces with opposite normals" << endl;

    if( (nBadVertices != 0) && removeConnections )
    {
        clearMeshEngine();
...
)
{
    ...
    # ifdef USE_OMP
    # pragma omp parallel for private(frontCells) schedule(dynamic, 100) \
    reduction(+ : nBadVertices)
    # endif
    for(label bpI=0;bpI<size;++bpI)
    {
        ...
        forAllRow(pointCells, pointI, cI)
        {
            ...
            while( frontCells.size() != 0 )
            {
                ...
                forAll(c, fI)
                {
                    label neiCell = owner[c[fI]];
                    if( neiCell == cLabel )
                        neiCell = neighbour[c[fI]];
        ...
        if( nGroup != 1 )
        {
            ...
            # ifdef USE_OMP
            # pragma omp critical(cellGroupsAtBndVertices)
            # endif
            {
...
)
{
    ...
    # ifdef USE_OMP
    # pragma omp parallel for private(front) schedule(guided, 100)
    # endif
    for(label bpI=0;bpI<size;++bpI)
    {
    ...
    if( Pstream::parRun() )
    {
        ...
        const labelLongList& globalFaceLabel = mse.globalBoundaryFaceLabel();
