...
void meshOctreeCreator::setRootCubeSizeAndRefParameters()
{
    ...
    Info << "Reading refinement settings" << endl;
    ...
    //- adjust the bounding box such that it matches the requested
    //- cell size in the contact region
    if( meshDictPtr_->found("contactCellSize") )
    {
        const scalar contactSize =
            readScalar(meshDictPtr_->lookup("contactCellSize"));

        direction level = globalRefLevel_;
        do
        {
            finished = false;

            const scalar lSize = size / Foam::pow(2, label(level));

            if( lSize < (contactSize * (1.0+SMALL)) )
            {
                const scalar bbSize =
                    0.5 * contactSize * Foam::pow(2, label(level));
    ...
    if( meshDictPtr_->found("localRefinement") )
    {
        if( meshDictPtr_->isDict("localRefinement") )
        {
            ...
            const wordList patchNames = surface.patchNames();
            std::map<word, label> patchToIndex;
            forAll(patchNames, patchI)
                patchToIndex[patchNames[patchI]] = patchI;

            List<std::set<label> > facetsInPatch(patchNames.size());
            forAll(surface, triI)
                facetsInPatch[surface[triI].region()].insert(triI);
            ...
            forAll(entries, dictI)
            {
                ...
                std::map<word, label>::const_iterator pIt =
                    patchToIndex.find(pName);
                if( pIt != patchToIndex.end() )
                {
                    const label patchI = pIt->second;

                    forAllConstIter
                    (
                        std::set<label>,
                        facetsInPatch[patchI],
                        it
                    )
                    {
                        surfRefLevel_[*it].append(rp);
                    }
                }
                else
                {
                    const labelList matchedPatches =
                        findStrings(pName, patchNames);
    ...
    Info << "Finished reading refinement settings" << endl;
}
...
void meshOctreeCreator::createOctreeBoxes()
{
    ...
    {
        meshOctreeAutomaticRefinement autoRef
        (
            octree_,
            *meshDictPtr_,
            globalRefLevel_,
    ...
    //- Add an additional layer of octree leaves from other processors
    if( Pstream::parRun() )
    {
        loadDistribution(true);

        //- add additional layer fo leaves from other processors
        meshOctreeModifier om(octree_);
        om.addLayerFromNeighbouringProcessors();
    }

    meshOctreeModifier(octree_).reduceMemoryConsumption();
}
...
)
{
    ...
    octreeModifier.searchRangeAccess() =
        (rootBox.max().x() - rootBox.min().x()) / pow(2.0, label(maxLevel));
    ...
    while( true )
    {
        ...
        refineBox.setSize(leaves.size());

        label nMarked(0);

        # ifdef USE_OMP
        # pragma omp parallel for schedule(dynamic, 100) \
        reduction(+ : nMarked) private(ct)
        # endif
        forAll(leaves, leafI)
        {
        ...
        Info << "Refined " << nMarked << " leaves" << endl;
    }
}
