...
triSurf::triSurf(const triSurf& s)
:
    triSurfPoints(s),
    triSurfFacets(s),
    triSurfFeatureEdges(s),
    triSurfAddressing(triSurfPoints::points_, triSurfFacets::triangles_)
{}
...
sizeType triSurf::printAllocated() const
{
    sizeType totalMemUsage(sizeof(triSurf));

    totalMemUsage += triSurfPoints::points_.byteSize();

    totalMemUsage += sizeof(labelledTri) * triangles_.size();

    totalMemUsage += sizeof(edge) * featureEdges_.size();

    forAllConstIter(Map<meshSubset>, pointSubsets_, it)
    {
        totalMemUsage += sizeof(label);
        totalMemUsage += it().byteSize();
    }
    ...
    totalMemUsage += triSurfAddressing::printAllocated();

    Pout << "triSurf: mem usage " << totalMemUsage/1048576 << " Mb." << endl;
