...
bool checkClosedBoundary(const polyMeshGen& mesh, const bool report)
{
    ...
    for(label faceI = mesh.nInternalFaces();faceI<faces.size();++faceI)
    {
        const vector fArea = help::faceAreaVector(points, faces[faceI]);
        sumClosed += fArea;
        sumMagClosedBoundary += mag(fArea);
    }
...
)
{
    ...
    vectorLongList sumClosed(cells.size(), vector::zero);

    scalarLongList sumMagClosed(cells.size(), 0.0);

    const labelLongList& own = mesh.owner();
    const labelLongList& nei = mesh.neighbour();

    forAll(own, faceI)
    {
        const vector fArea = help::faceAreaVector(points, faces[faceI]);
        // Add to owner
        sumClosed[own[faceI]] += fArea;
        sumMagClosed[own[faceI]] += mag(fArea);

        if( nei[faceI] == -1 )
            continue;

        // Subtract from neighbour
        sumClosed[nei[faceI]] -= fArea;
        sumMagClosed[nei[faceI]] += mag(fArea);
    }
    ...
    const scalarLongList& vols = mesh.addressingData().cellVolumes();
...
)
{
    ...
    # endif
    {
        ...
        forAll(faces, faceI)
        {
            ...
            const scalar magFaceArea =
                mag(help::faceAreaVector(points, faces[faceI]));
...
)
{
    ...
    const labelLongList& neighbour = mesh.neighbour();

    const vectorLongList& cCentres = mesh.addressingData().cellCentres();
...

{
    ...
    const vectorLongList& centres = mesh.addressingData().cellCentres();
    const pointFieldPMG& points = mesh.points();
    ...
    # endif
    {
        ...
        for(label faceI=0; faceI<nInternalFaces;++faceI)
        {
            ...
            const face& f = faces[faceI];
            const point fCentre = help::faceCentre(points, f);
    ...
    # endif
    {
        ...
        forAll(cells, cI)
        {
            if( affectedCells[cI] )
            {
                ...
                const vectorLongList& areas = mesh.addressingData().faceAreas();
...
)
{
    ...
    schedule(guided, 100) reduction(+ : nNegVolCells)
    # endif
    forAll(owner, faceI)
    {
        if( changedFacePtr && !(*changedFacePtr)[faceI] )
...
)
{
    ...
    # endif
    {
        ...
        for(label faceI=0;faceI<nInternalFaces;++faceI)
        {
            ...
            const vector s = help::faceAreaVector(points, faces[faceI]);
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            # endif
            {
                ...
                forAll(otherCentres, fI)
                {
                    ...
                    const vector& s =
                        help::faceAreaVector(points, faces[faceI]);
...
)
{
            ...
            if( dDotS < severeNonorthogonalityThreshold )
            {
                if( dDotS > SMALL )
                {
                    if( report )
                    {
                        ...
                        # ifdef USE_OMP
                        # pragma omp critical(reportDotS)
                        # endif
    ...
    if( Pstream::parRun() )
    {
        ...
        # ifdef USE_OMP
        # pragma omp parallel \
        reduction(+ : severeNonOrth, errorNonOrth, sumDDotS, nParFaces)
        # endif
        {
            scalar localMinDDotS(VGREAT);
        ...
        //- count the number of faces at inter-prcessor boundaries
        //- divide the number by 2 because each face appears at two processors
        counter += returnReduce(nParFaces, sumOp<label>()) / 2;
    }
...
)
{
    ...
    const vectorLongList& ctrs = mesh.addressingData().cellCentres();
    ...
    # ifdef USE_OMP
    # pragma omp parallel for schedule(guided, 100) reduction(+ : nErrorPyrs)
    # endif
    forAll(faces, faceI)
    {
        if( changedFacePtr && !(*changedFacePtr)[faceI] )
        ...
        const scalar pyrVol =
            pyramidPointFaceRef(faces[faceI], ctrs[owner[faceI]]).mag(points);
        ...
        if( pyrVol > -minPyrVol )
        {
            ...
            {
                # ifdef USE_OMP
                # pragma omp critical(reportBadPyramids)
                # endif
        ...
        if( badFace )
        {
            ...
            {
                # ifdef USE_OMP
                # pragma omp critical(storeBadFaces)
                # endif
                setPtr->insert(faceI);
            }
...
)
{
    ...
    for(label faceI=0;faceI<nInternalFaces;++faceI)
    {
        ...
        const point fCentre = help::faceCentre(points, faces[faceI]);

        const scalar dOwn = mag(fCentre - centres[own[faceI]]);
        const scalar dNei = mag(fCentre - centres[nei[faceI]]);
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            # endif
            {
                ...
                forAll(otherCentres, fI)
                {
                    ...
                    const point fCentre =
                        help::faceCentre(points, faces[faceI]);
                    const point& cOwn = centres[own[faceI]];
                    const point& cNei = otherCentres[fI];
                    const scalar dOwn = mag(fCentre - cOwn);
                    const scalar dNei = mag(fCentre - cNei);
    ...
    forAll(boundaries, patchI)
    {
        ...
        {
            const point fCentre = help::faceCentre(points, faces[faceI]);
            const vector d = fCentre - centres[own[faceI]];
    ...
    # endif
    {
        ...
        forAll(faceSkewness, faceI)
        {
            ...
            if( skewness > warnSkew )
            {
                ...
                {
                    # ifdef USE_OMP
                    # pragma omp critical(reportSkewness)
                    # endif
                ...
                {
                    # ifdef USE_OMP
                    # pragma omp critical(badSkewness)
                    # endif
                    setPtr->insert(faceI);
                }
...
)
{
    ...
    for(label faceI=0;faceI<nInternalFaces;++faceI)
    {
        ...
        const scalar dOwn
        (
            Foam::mag(centres[owner[faceI]] - fCentre)
        );
        const scalar dNei
        (
            Foam::mag(centres[neighbour[faceI]] - fCentre)
        );
    ...
    if( Pstream::parRun() )
    {
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            forAll(dst, faceI)
            {
                ...
                const point fCentre = help::faceCentre(points, faces[fI]);
                dst[faceI] = Foam::mag(centres[owner[fI]] - fCentre);
            }
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            forAll(otherDst, faceI)
            {
                ...
                const point fCentre = help::faceCentre(points, faces[fI]);
                const scalar dOwn =
                    Foam::mag(centres[owner[fI]] - fCentre);
...
)
{
    ...
    # endif
    {
        ...
        forAll(faces, faceI)
        {
            ...
            const face& f = faces[faceI];
            const vector fArea = help::faceAreaVector(points, f);

            const vector fNormal = fArea / (mag(fArea)+VSMALL);
        ...
        # ifdef USE_OMP
        # pragma omp critical(findMaxSin)
        # endif
...
{
    if( warnFlatness < 0.0 || warnFlatness > 1.0 )
    {
        FatalErrorIn
        (
            "bool checkFaceFlatness("
    ...
    # endif
    {
        ...
        forAll(faces, faceI)
        {
            ...
            if( (f.size() > 3) && (mag(fArea) > VSMALL) )
            {
                const point fc = help::faceCentre(points, f);
                ...
                const scalar flatness = mag(fArea) / (sumA+VSMALL);
                ...
                if( flatness < warnFlatness )
                {
                    ...
                    {
                        # ifdef USE_OMP
                        # pragma omp critical(insertToSet)
                        # endif
                        setPtr->insert(faceI);
                    }
                }
        ...
        # ifdef USE_OMP
        # pragma omp critical(reduceMinFlatness)
        # endif
        {
...
)
{
    ...
    //- maximum face concavity
    scalar maxConcave = 10;
    ...
    if( mesh.returnTime().foundObject<dictionary>("meshDict") )
    {
        const dictionary& meshDict =
            mesh.returnTime().lookupObject<dictionary>("meshDict");
        ...
        if( meshDict.found("meshQualitySettings") )
        {
            ...
            if( qualityDict.found("minTetQuality") )
            {
                ...
                Info << "Checking minTetQuality " << minTetQuality << endl;
            ...
            //- reading max concave face angle
            if( qualityDict.found("maxConcave") )
            {
                maxConcave = readScalar(qualityDict.lookup("maxConcave"));
