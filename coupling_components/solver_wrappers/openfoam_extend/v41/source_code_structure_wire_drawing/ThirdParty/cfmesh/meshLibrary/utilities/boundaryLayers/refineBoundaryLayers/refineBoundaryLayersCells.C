...
template<class cellGraph>
void writeCellsToVTK
(
...
)
{
    ...
    DynList<point> pts;
    std::map<label, label> newLabel;
    forAll(cells, ncI)
    {
        forAll(cells[ncI], fI)
            forAll(cells[ncI][fI], pI)
            {
                if( newLabel.find(cells[ncI][fI][pI]) == newLabel.end() )
                {
                    newLabel[cells[ncI][fI][pI]] = pts.size();
                    pts.append(points[cells[ncI][fI][pI]]);
                }
            }
    }

    file << "POINTS " << pts.size() << " float\n";
    forAll(pts, pI)
    {
    ...
    forAll(cells, ncI)
    {
        forAll(cells[ncI], fI)
        {
            file << cells[ncI][fI].size();

            forAll(cells[ncI][fI], pI)
                file << " " << newLabel[cells[ncI][fI][pI]];
...
) const
{
    ...
    forAll(c, fI)
    {
        ...
        else
        {
            ...
            //- TODO: this does not work when the layer is collapsed
            const bool shareAnEdge =
                help::shareAnEdge(faces[c[baseFace]], faces[c[fI]]);
            if( !shareAnEdge )
            {
                //- face is the other base face. It does not share an edge
                //- with the base face
                const label faceI = facesFromFace_(c[fI], 0);
                otherBaseFace = fI;
                DynList<label, 8> f;
        ...
        forAll(nAppearances, eI)
            if( nAppearances[eI] != 2 )
            {
                ...
                (
                    mesh().points(),
                    cellsFromCell,
                    "cellsFromCell_"+help::labelToText(cellI)+".vtk"
                );
                //abort(FatalError);
            }
    }
...
{
    const labelLongList& nLayersAtBndFace = bndLayers_.nLayersAtBndFace_;
    ...
    forAll(c, fI)
    {
        ...
        if( bndLayers_.faceInLayer_[bfI] < 0 )
            continue;
    ...
    # ifdef DEBUGLayer
    Info << "nLayersI " << nLayersI_ << endl;
    Info << "nLayersJ " << nLayersJ_ << endl;
    # endif
...
void refineBoundaryLayers::refineEdgeHexCell::populateExistingFaces()
{
    ...
    forAllRow(facesFromFace, c[faceInDirection_[2]], i)
    {
        ...
        cellsFromCell_[min(i, maxI)].append(newFaces[faceI]);
    }
    ...
    forAllRow(facesFromFace, c[faceInDirection_[3]], i)
    {
        ...
        cellsFromCell_[min(i, maxI) + maxJ * nLayersI_].append(newFaces[faceI]);
    }
    ...
    forAllRow(facesFromFace, c[faceInDirection_[4]], j)
    {
        ...
        cellsFromCell_[min(j, maxJ) * nLayersI_].append(newFaces[faceI]);
    }
    ...
    forAllRow(facesFromFace, c[faceInDirection_[5]], j)
    {
        ...
        cellsFromCell_[min(j, maxJ) * nLayersI_ + maxI].append(newFaces[faceI]);
    }
    ...
    forAll(cellsFromCell_, cI)
    {
        ...
        forAll(nAppearances, eI)
            if( nAppearances[eI] != 2 )
            {
                ...
                writeCellsToVTK
                (
                    bndLayers_.mesh().points(),
                    cellsFromCell_,
                    "cellsFromCell_"+help::labelToText(cellI_)+".vtk"
                );
                //abort(FatalError);
            }
    }
...
void refineBoundaryLayers::refineCornerHexCell::generateNewPoints()
{
    ...
    const pointFieldPMG& points = bndLayers_.mesh_.points();
    ...
    for(label i=1;i<nLayersI_;++i)
    {
        ...
        for(label j=1;j<nLayersJ_;++j)
        {
            ...
            for(label k=1;k<nLayersK_;++k)
            {
                ...
                bndLayers_.mesh_.appendVertex(newP);
            }
        }
    }
...
{
    labelLongList nCellsFromCell(mesh_.cells().size(), 1);
    labelLongList refType(mesh_.cells().size(), 0);

    const meshSurfaceEngine& mse = surfaceEngine();
    const labelLongList& faceOwners = mse.faceOwners();
    ...
    forAll(nCellsFromCell, cellI)
    {
        ...
        else if( refType[cellI] == 3 )
        {
            ...
            //- check whether there exist a point with backup
            label backupPoint(-1);
            forAll(cells[cellI], fI)
            {
                const face& f = faces[cells[cellI][fI]];

                forAll(f, pI)
                {
                    if( mesh_.hasBackup(f[pI]) )
                    {
                        backupPoint = f[pI];
                        break;
                    }
            ...
            pointNewFaces.setSize(points.size());
    ...
    mesh_.updateCellZones(newCellsFromCell);
    ...
    mesh_.updateFaceZones(facesFromFace_);
    ...
    if( specialMode_ )
    {
        //- cleanup boundaryLayerAtPatch_ subsets
        //- only the cells attached to the patch shall remain in subsets
        const cellListPMG& cells = mesh_.cells();
        const PtrList<boundaryPatch>& patches = mesh_.boundaries();
        forAll(patches, patchI)
        {
            const word name = "boundaryLayerAtPatch_"+help::labelToText(patchI);
            const label cId = mesh_.cellSubsetIndex(name);
            ...
            //- get cells in this subset
            labelLongList cellsInSubset;
            mesh_.cellsInSubset(cId, cellsInSubset);

            forAll(cellsInSubset, i)
            {
                const label cellI = cellsInSubset[i];
                const cell& c = cells[cellI];

                bool foundPatchFace(false);
                forAll(c, fI)
                {
                    if( (c[fI] >= start) && (c[fI] < end) )
                    {
                        foundPatchFace = true;
                        break;
                    }
                ...
                if( !foundPatchFace )
                    mesh_.removeCellFromSubset(cId, cellI);
            }
        }
    }

    # ifdef DEBUGLayer
    const label openCellsId = mesh_.addCellSubset("openCells");
    const label openPointsId = mesh_.addPointSubset("openPoints");
    for(label procI=0;procI<Pstream::nProcs();++procI)
    {
        if( procI == Pstream::myProcNo() )
        {
            forAll(cells, cellI)
            {
                ...
                if( badCell )
                {
                    mesh_.addCellToSubset(openCellsId, cellI);
                    Pout << "Cell " << cellI
                    ...
                    forAll(edges, eI)
                        if( nAppearances[eI] != 2 )
                        {
                            ...
                            mesh_.addPointToSubset(openPointsId, e.start());
                            mesh_.addPointToSubset(openPointsId, e.end());
                        }
