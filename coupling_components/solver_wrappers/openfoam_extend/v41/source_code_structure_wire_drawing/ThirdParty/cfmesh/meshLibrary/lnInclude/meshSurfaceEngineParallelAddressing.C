...
void meshSurfaceEngine::calcGlobalBoundaryPointLabels() const
{
    if( !globalBoundaryPointLabelPtr_ )
        globalBoundaryPointLabelPtr_ = new labelLongList();

    const labelLongList& bPoints = this->boundaryPoints();
    labelLongList& globalPointLabel = *globalBoundaryPointLabelPtr_;
...
void meshSurfaceEngine::calcGlobalBoundaryEdgeLabels() const
{
    if( !globalBoundaryEdgeLabelPtr_ )
        globalBoundaryEdgeLabelPtr_ = new labelLongList();

    const edgeLongList& bEdges = this->edges();

    labelLongList& globalEdgeLabel = *globalBoundaryEdgeLabelPtr_;
...
{
    const labelLongList& globalEdgeLabel = this->globalBoundaryEdgeLabel();
    const labelLongList& boundaryFacePatches = this->boundaryFacePatches();
...
void meshSurfaceEngine::calcGlobalBoundaryFaceLabels() const
{
    ...
    if( !globalBoundaryFaceLabelPtr_ )
        globalBoundaryFaceLabelPtr_ = new labelLongList(bFaces.size());

    labelLongList& globalFaceLabel = *globalBoundaryFaceLabelPtr_;
