...
) const
{
    ...
    const scalar tol = SMALL * cubeBox.mag();
    cubeBox.min() -= point(tol, tol, tol);
    cubeBox.max() += point(tol, tol, tol);
...
void meshOctreeCube::leavesIntersectingLine
(
    const boundBox& rootBox,
    const point& startPoint,
    const point& endPoint,
    DynList<label>& markedLeaves
) const
{
    if( this->intersectsLine(rootBox, startPoint, endPoint) )
    {
        if( this->isLeaf() )
        {
            markedLeaves.append(this->cubeLabel());
        }
        else
        {
            for(label scI=0;scI<8;++scI)
            {
                ...
                else if( Pstream::parRun() )
                {
                    ...
                    if( cc.intersectsLine(rootBox, startPoint, endPoint) )
                        markedLeaves.append(meshOctreeCube::OTHERPROC);
                }
            }
        }
    }
...
void meshOctreeCube::purgeContainedElements()
{
    if( this->isLeaf() )
        return;
    ...
    if( cleanup )
    {
        meshOctreeSlot& slot = *(this->activeSlotPtr_);

        if( containedElementsLabel_ != -1 )
            slot.containedTriangles_.setRowSize(containedElementsLabel_, 0);
        if( containedEdgesLabel_ != -1 )
            slot.containedEdges_.setRowSize(containedEdgesLabel_, 0);
    }
}
