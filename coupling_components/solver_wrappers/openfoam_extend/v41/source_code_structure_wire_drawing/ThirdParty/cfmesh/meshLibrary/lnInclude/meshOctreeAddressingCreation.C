...
void meshOctreeAddressing::markBOUNDARYLeaves() const
{
...
void meshOctreeAddressing::createOctreePoints() const
{
    ...
    for(label cubeI=0;cubeI<nLeaves;++cubeI)
    {
        ...
        oc.vertices(rootBox, vertices, 0.0);

        forAllRow(nodeLabels, cubeI, nI)
        {
            const label nodeI = nodeLabels(cubeI, nI);

            //- find the cube with the minimum label
            label minCubeLabel = cubeI;
            forAllRow(nodeLeaves, nodeI, nlI)
            {
                const label leafI = nodeLeaves(nodeI, nlI);

                if( (leafI < 0) || !nodeLabels.contains(leafI, nodeI) )
                    continue;

                minCubeLabel = min(minCubeLabel, leafI);
            }

            if( minCubeLabel == cubeI )
            {
...
void meshOctreeAddressing::createNodeLabels() const
{
    ...
    nNodes_ = 0;
    DynList<label> numLocalNodesAtTask;
    # ifdef USE_OMP
    # pragma omp parallel
    # endif
    {
        ...
        # endif
        {
            numLocalNodesAtTask.setSize(nTasks);

            //- count the number of nodes local to each process
            forAll(numLocalNodesAtTask, taskI)
            {
                # ifdef USE_OMP
                # pragma omp task shared(numLocalNodesAtTask, nodeLabels) \
                shared(boxType) firstprivate(taskI)
                # endif
                {
                    label& nLocalNodes = numLocalNodesAtTask[taskI];
                    nLocalNodes = 0;

                    const label chSize = max(nodeLabels.size() / nTasks, 1);
                    const label sc = taskI * chSize;
                    label ec = taskI!=(nTasks-1)?sc+chSize:boxType.size();

                    ec = min(ec, boxType.size());

                    for(label leafI=sc;leafI<ec;++leafI)
                    {
                        forAllRow(nodeLabels, leafI, nI)
                        {
                            ...
                            forAll(pLeaves, plI)
                            {
                                if( pLeaves[plI] > -1 )
                                {
                                    ...
                                    if( validLeaf[plI] )
                                        minLeaf = min(minLeaf, pLeaves[plI]);
                                }
                                else
                                {
                                    validLeaf[plI] = false;
                            ...
                            if( (minLeaf == leafI) && validLeaf[7-nI] )
                            {
                                forAll(pLeaves, plI)
                                    if( validLeaf[plI] )
                                    {
                                        //- set node labels to -2
                                        //- not to repeat searches
        ...
        # endif
        {
            ...
            forAll(numLocalNodesAtTask, taskI)
            {
                nNodes_ += numLocalNodesAtTask[taskI];

                # ifdef USE_OMP
                # pragma omp task shared(nodeLabels, boxType) \
                firstprivate(taskI, startNode)
                # endif
                {
                    const label chSize = max(nodeLabels.size() / nTasks, 1);
                    const label sc = taskI * chSize;
...
void meshOctreeAddressing::findGroupsOfUsedBoxes() const
{
    const List<direction>& boxType = this->boxType();

    boxGroupPtr_ = new labelList(boxType.size(), -1);

    nGroups_ =
        help::groupMarking
        (
            *boxGroupPtr_,
            meshOctreeOps::meshOctreeNeighbourOperator(octree_),
            meshOctreeOps::meshOctreeSelectorOperator
            (
                boxType,
    ...
    Info << "Found " << nGroups_
         << " independent groups of leaves used in the mesh" << endl;
}
                ...
                else if( boxType[leafI] & BOUNDARY )
                {
                    for(label fI=0;fI<6;++fI)
                    {
                        ...
                        )
                        {
                            ...
                            helperFaces.appendList(help::reverseFace(cf));
                            helperOwner.append(nei);
                            helperNeighbour.append(leafI);
                        }
                    }
