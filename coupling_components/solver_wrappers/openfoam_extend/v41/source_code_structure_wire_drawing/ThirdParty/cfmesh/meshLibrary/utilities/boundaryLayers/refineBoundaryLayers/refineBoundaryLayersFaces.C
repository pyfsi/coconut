...
)
{
    ...
    if( (nLayersInDirection[0] <= 1) && (nLayersInDirection[1] <= 1) )
    {
        ...
        # ifdef DEBUGLayer
        Info << "1. No refinement: Orig face " << f
             << " new face " << newF << endl;
        # endif
    ...
    if( (dir0 < 0) && (dir1 < 0) )
    {
        ...
        {
            OFstream file("faceCannotBeSplit.vtk");
            ...
            forAll(f, pI)
            {
                const point& p = mesh_.points()[f[pI]];
...
//        Pout << "Point " << pointI
//             << " coordinates " << points[pointI] << endl;
    Pout << "v00 = " << v00 << endl;
    Pout << "v10 = " << v10 << endl;
    Pout << "v11 = " << v11 << endl;
    Pout << "v01 = " << v01 << endl;
...
bool refineBoundaryLayers::findNumberOfSplits
(
    const label faceI,
    FixedList<label, 2>& nSplits
) const
{
    nSplits = 1;
    ...
    activeCells.append(mesh_.owner()[faceI]);
    if( faceI < nIntFaces )
        activeCells.append(mesh_.neighbour()[faceI]);

    bool isSplit(false);

    forAll(activeCells, cI)
    {
        const cell& c = cells[activeCells[cI]];
        ...
        forAll(c, fI)
        {
            ...
            const label bfI = c[fI] - nIntFaces;
            ...
            {
                //- check if the face must be split and in which direction
                const face& bf = faces[c[fI]];

                forAll(f, eI)
                {
                    const edge e = f.faceEdge(eI);

                    if( help::positionOfEdgeInFace(e, bf) >= 0 )
                    {
                        //- faces share this edge
                        const label dir = eI % 2;
                        nSplits[dir] =
                            Foam::max(nSplits[dir], nLayersAtBndFace_[bfI]);
                    }
                }

                //- face is part of the layer
                isSplit = true;
            }
        }
    }
    ...
    # ifdef DEBUGLayer
    Info << "Number of splits for face " << faceI << " is " << nSplits << endl;
    # endif
...
void refineBoundaryLayers::generateNewFaces()
{
    ...
    for(label faceI=0;faceI<nInternalFaces;++faceI)
    {
        ...
        FixedList<label, 2> nSplits;
        if( !findNumberOfSplits(faceI, nSplits) )
        {
            facesFromFace_.append(faceI, newFaces_.size());
            newFaces_.appendList(f);
            continue;
        ...
        refineFace(f, nSplits, newFacesForFace);
    ...
    forAll(bFaces, bfI)
    {
        ...
        //- refine the face
        DynList<DynList<label, 4>, 128> newFacesForFace;
        refineFace(bf, nSplits, newFacesForFace);
    ...
    if( Pstream::parRun() )
    {
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            for(label fI=0;fI<size;++fI)
            {
                ...
                FixedList<label, 2> nSplits;
                findNumberOfSplits(faceI, nSplits);

                forAll(nSplits, dirI)
                {
                    localSplits[faceI].append(labelPair(dirI, nSplits[dirI]));

                    if( nSplits[dirI] > 1 )
                    {
                        sendData.append(fI);
                        sendData.append(dirI);
                        sendData.append(nSplits[dirI]);
                    }
                }
            }
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            for(label fI=0;fI<size;++fI)
            {
                ...
                if( (it == localSplits.end()) || (f.size() != 4) )
                {
                ...
                if( procBoundaries[patchI].owner() )
                {
                    ...
                    refineFace(f, nLayersInDirection, facesFromFace);
                ...
                else
                {
                    ...
                    const face rFace = f.reverseFace();
