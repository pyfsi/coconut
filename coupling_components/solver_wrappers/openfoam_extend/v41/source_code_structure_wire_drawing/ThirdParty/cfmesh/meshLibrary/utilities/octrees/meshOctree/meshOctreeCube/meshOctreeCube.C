...
(
    const meshOctreeCubeCoordinates& cc,
    const DynList<label>& containedTriangles,
    meshOctreeSlot* slotPtr
)
:
    meshOctreeCubeBasic(cc),
...
{
    slotPtr->containedTriangles_.setSize(1);
    slotPtr->containedTriangles_.setRowSize(0, containedTriangles.size()),
    slotPtr->containedEdges_.setSize(0);

    forAll(containedTriangles, i)
        slotPtr->containedTriangles_(0, i) = containedTriangles[i];
}
