...
void cartesianMeshExtractor::createPolyMesh()
{
    ...
    //- assign cell labels to octree boxes
    ...
    labelList owner, neighbour;
    faceList octreeFaces;
    octreeCheck_.createFaces(octreeFaces, owner, neighbour);
    ...
    forAll(owner, faceI)
    {
        ...
        const label neiLabel = nei>=0?leafCellLabel[nei]:-1;
        ...
        else if( ownLabel != -1 )
        {
            ...
            (
                (nei != -1) &&
                (cType[nei] & meshOctreeAddressing::MESHCELL)
            )
            {
        ...
        else if( neiLabel != -1 )
        {
            ...
            (
                (own != -1) &&
                (cType[own] & meshOctreeAddressing::MESHCELL)
            )
            {
    ...
    //- clear out
    octreeCheck_.clearFacesAndNodeLabels();
    ...
    if( Pstream::parRun() )
    {
        ...
        for(iter=procFaces.begin();iter!=procFaces.end();++iter)
        {
            ...
            forAll(patchFaces, pfI)
            {
                ...
                {
                    //- add a face
                    faces[nProcBoundaries].transfer(octreeFaces[fLabel]);
                    cells[curCell][nFacesInCell[curCell]++] = nProcBoundaries++;
                }
                else if( curCell == -1 )
                {
                    //- add a reversed face
                    faces[nProcBoundaries] = octreeFaces[fLabel].reverseFace();
                ...
                else
                {
                    ...
                    (
                        "void cartesianMeshExtractor::createPolyMesh()"
                    ) << "Face " << faces[fLabel] << " causes problems!"
                        << abort(FatalError);
                }
            }
    ...
    forAll(owner, faceI)
    {
        ...
        {
            //- internal face
            faces[nFaces].transfer(octreeFaces[faceI]);
        ...
        else if( neiLabel != -1 )
        {
            ...
            //- boundary face
            faces[nFaces] = octreeFaces[faceI].reverseFace();
    ...
    # ifdef DEBUGMesh
    const label innerId = mesh_.addCellSubset("INSIDE");
    const label bndId = mesh_.addCellSubset("BOUNDARY");
    const meshOctree& octree = octreeCheck_.octree();

    forAll(leafCellLabel, leafI)
    {
        if( leafCellLabel[leafI] < 0 )
            continue;

        if( octree.returnLeaf(leafI).cubeType() & meshOctreeCube::INSIDE )
            mesh_.addCellToSubset(innerId, leafCellLabel[leafI]);
        if( octree.returnLeaf(leafI).cubeType() & meshOctreeCube::DATA )
            mesh_.addCellToSubset(bndId, leafCellLabel[leafI]);
    }
    ...
    vectorLongList closedness(cells.size(), vector::zero);
