...
public:
    ...
    void operator()(const label bfI, DynList<label>& neighbourFaces) const
    {
        ...
        const labelLongList& faceOwner = mse_.faceOwners();
...
void detectBoundaryLayers::analyseLayers()
{
    ...
    const PtrList<boundaryPatch>& boundaries = mesh.boundaries();
    const labelLongList& facePatch = meshSurface_.boundaryFacePatches();

    //- check for the existence of extra patches. Boundary layers cannot exist
    //- at those patches
    wordHashSet extraPatches;
    if( mesh.metaData().found("extraPatches") )
        extraPatches = HashSet<word>(mesh.metaData().lookup("extraPatches"));

    //- find patches where the boundary layer was generated
    wordHashSet boundaryLayerAtPatches;
    if( mesh.metaData().found("boundaryLayerAtPatches") )
        boundaryLayerAtPatches =
            HashSet<word>(mesh.metaData().lookup("boundaryLayerAtPatches"));
    ...
    //- find existing layers in the mesh
    nFirstLayers_ =
        help::groupMarking
        (
            layerAtBndFace_,
    ...
    //- ensure that the faces where the layer was not extruded
    //- have the negative layer id
    boolList activePatches(mesh.boundaries().size(), true);
    forAll(activePatches, patchI)
    {
        const word pName = mesh.boundaries()[patchI].patchName();

        if( !boundaryLayerAtPatches.found(pName) )
            activePatches[patchI] = false;
    }

    forAll(layerAtBndFace_, bfI)
    {
        if( !activePatches[facePatch[bfI]] )
    ...
    if( mesh.returnTime().foundObject<dictionary>("meshDict") )
    {
        ...
        if( meshDict.found("boundaryLayers") )
        {
            ...
            //- the user can override the default layer topology
            //- for inlet and outlet patches
            if( bndLayers.isDict("inletOutletPatches") )
            {
                const dictionary& inletOutletDict =
                    bndLayers.subDict("inletOutletPatches");

                const wordList patchNames = inletOutletDict.toc();

                boolList noLayerAtPatch(boundaries.size(), false);
                ...
                forAll(patchNames, i)
                {
                    ...
                    forAll(boundaries, j)
                    {
                        if( boundaries[j].patchName() == patchNames[i] )
                        {
                            patchI = j;
                            break;
                        }
                    ...
                    if( patchI >= boundaries.size() || (patchI < 0) )
                    {
                        Warning << "Patch " << patchNames[i]
                            << " does not exist "
                            << "as a patch name " << boundaries << endl;
                        continue;
                    }

                    const dictionary& pDict =
                        inletOutletDict.subDict(patchNames[i]);

                    bool generateLayer(true);
                    if( pDict.found("generateLayer") )
                        generateLayer = readBool(pDict.lookup("generateLayer"));

                    if( !generateLayer )
                    {
                        noLayerAtPatch[patchI] = true;
                    }
                }

                forAll(layerAtBndFace_, bfI)
                {
                    if( noLayerAtPatch[facePatch[bfI]] )
                        layerAtBndFace_[bfI] = -1;
                }
            }

            //- the user can specify a global setting for symmetry plane
            //- ensuring that boundary layers exit out at the symmetry plane
            //- in some cases it is not possible to extrude boundary layers at
            //- symmetry plane patches
            if( bndLayers.found("symmetryPlaneLayerTopology") )
            {
                const bool symmPlaneLayerTopo
                (
                    readBool(bndLayers.lookup("symmetryPlaneLayerTopology"))
                );

                if( symmPlaneLayerTopo )
                {
                    bool generateLayersAtSymmetryPlanes(true);
                    if( bndLayers.found("generateLayersAtSymmetryPlanes") )
                        generateLayersAtSymmetryPlanes =
                            readBool
                            (
                                bndLayers.lookup
                                (
                    ...
                    {
                        const word pType = boundaries[patchI].patchType();

                        if
                        (
                            (pType == "symmetry") || (pType == "symmetryPlane")
                        )
                        {
                            if( !generateLayersAtSymmetryPlanes )
                            {
                                noLayerAtPatch[patchI] = true;
                            }
                        }
...
void detectBoundaryLayers::generateHairEdges()
{
    ...
    List<edgeLongList> edgesAtTask;
    labelList startAtTask;
    ...
    {
        # ifdef USE_OMP
        const label nTasks = omp_get_num_threads() * 3;
        # pragma omp single
        {
            edgesAtTask.setSize(nTasks);
        }
        # else
        ...
        # endif
        {
            ...
            {
                # ifdef USE_OMP
                # pragma omp task shared(bFaces, edgesAtTask) \
                firstprivate(taskI)
                # endif
                {
                    edgeLongList& localEdges = edgesAtTask[taskI];
                    localEdges.clear();

                    const label chSize = max(bFaces.size() / nTasks, 1);
                    const label sf = taskI * chSize;
                    const label ef = taskI!=(nTasks-1)?sf+chSize:bFaces.size();

                    for(label bfI=sf;bfI<ef;++bfI)
                    {
                        if( layerAtBndFace_[bfI] < 0 )
                            continue;
        ...
        # pragma omp single
        {
            startAtTask.setSize(nTasks);
            label nEdges(0);
            forAll(edgesAtTask, taskI)
            {
                startAtTask[taskI] = nEdges;
                nEdges += edgesAtTask[taskI].size();
            }

            hairEdges_.setSize(nEdges);

            forAll(edgesAtTask, taskI)
            {
                # ifdef USE_OMP
                # pragma omp task shared(startAtTask, edgesAtTask) \
                firstprivate(taskI)
                # endif
                {
                    label& edgeI = startAtTask[taskI];

                    const edgeLongList& edges = edgesAtTask[taskI];

                    forAll(edges, eI)
                        hairEdges_[edgeI++] = edges[eI];
                }
            }
        }
        # else
        //- just transfer the data to splitEdges_
        hairEdges_.transfer(edgesAtTask[0]);
        # endif
    }
