...
void boundaryLayers::createNewVertices(const boolList& treatPatches)
{
    ...
    meshModifier.pointsAccess().setSize(points.size() + nExtrudedVertices);
    ...
    forAll(bPoints, bpI)
        if( patchVertex[bpI] )
        {
            ...
            (
                nPoints_,
                createNewVertex(bpI, treatPatches, patchVertex)
            );
    ...
    forAll(bPoints, bpI)
    {
        ...
        if( pLabel != -1 )
        {
            ...
            meshModifier.movePoint(pLabel, points[bPoints[bpI]]);

            //- unlock old surface points and lock the new ones
            if( mesh_.isLockedPoint(bPoints[bpI]) )
            {
                meshModifier.lockPoint(pLabel);
                meshModifier.unlockPoint(bPoints[bpI]);
            }

            meshModifier.movePoint(bPoints[bpI], p);
        }
    }
...
void boundaryLayers::createNewVertices(const labelList& patchLabels)
{
    ...
    meshModifier.pointsAccess().reserve(points.size() + 2 * bPoints.size());
    ...
    forAll(patchLabels, patchI)
    {
        ...
        forAll(patchPoints, i)
        {
            ...
            if( patchVertex[bpI] & EDGENODE )
            {
                ...
                meshModifier.movePoint(npI, p);
            }
            else
            {
                //- set the new point
                meshModifier.movePoint(newLabelForVertex_[pointI], p);
            }
        }

        if( Pstream::parRun() )
        {
            meshModifier.pointsAccess().setSize(nPoints_+procPoints.size());
    ...
    forAll(bPoints, bpI)
    {
        ...
        if( pLabel != -1 )
        {
            ...
            //- check if the orig point has its backup
            if( mesh_.hasBackup(bPoints[bpI]) )
            {
                point pOrig;
                mesh_.getOrigPoint(bPoints[bpI], pOrig);
                meshModifier.setBackupPoint(pLabel, pOrig);
                meshModifier.clearPointBackup(bPoints[bpI]);
            }
...
)
{
    ...
    forAll(procPoints, pointI)
    {
        ...
        if( !help::isnan(np) && !help::isinf(np) )
        {
            meshModifier.movePoint(nPoints_, np);
        }
        else
        {
            meshModifier.movePoint(nPoints_, p);
        }
...
)
{
    ...
    scalarLongList dist(edgePoints.size(), VGREAT);
    vectorLongList normal(edgePoints.size(), vector::zero);
    vectorLongList v(edgePoints.size(), vector::zero);
