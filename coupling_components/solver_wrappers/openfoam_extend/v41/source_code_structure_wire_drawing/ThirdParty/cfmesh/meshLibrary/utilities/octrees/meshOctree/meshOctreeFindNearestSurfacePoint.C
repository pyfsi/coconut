...
) const
{
    ...
    if( range > VSMALL )
    {
        sizeVec.x() = sizeVec.y() = sizeVec.z() = range;
    }
    else
    {
        ...
        if( cLabel >= 0 )
        {
            const scalar s = leaves_[cLabel]->size(rootBox_);
    ...
    do
    {
        //- set the squared range and the box for finding the nearest point
        distSq = sqr(sizeVec.x());
...
) const
{
    ...
    if( !found && !Pstream::parRun() )
    {
        Info << "Default search range " << searchRange_ << endl;
        Warning << "Could not find a boundary region for vertex " << p
            << " in patch " << region
            << " requested range " << range
            << " search range " << sizeVec << endl;
    }
}
    ...
    do
    {
        ...
        forAll(neighbours, neiI)
        {
            ...
            forAll(ce, eI)
            {
                ...
                DynList<label> foundPatches;
                forAllRow(edgeFaces, edgeI, efI)
                {
                    const label facetI = edgeFaces(edgeI, efI);

                    foundPatches.appendIfNotIn(surface_[facetI].region());
                ...
                if( !correctPatches || (foundPatches.size() < regions.size()) )
                    continue;
...
) const
{
    ...
    boundBox bb(c - sizeVec, c + sizeVec);
...
bool meshOctree::findSurfaceNormalAtNearestPoint
(
    vector& normal,
    const point& p,
...
) const
{
    ...
    if( patch >= 0 )
    {
        findNearestSurfacePointInRegion(np, dSq, nt, patch, p);
    }
    else
    {
        label region;
        findNearestSurfacePoint(np, dSq, nt, region, p);
    }

    //- create a search box with the centre at the nearest point
    const boundBox bb
    (
        np - vector(searchRadius, searchRadius, searchRadius),
        np + vector(searchRadius, searchRadius, searchRadius)
    );

    //- find the triangles within the range
    DynList<label> ct;
    findTrianglesInBox(bb, ct);
    ...
    forAll(ct, i)
    {
        const vector n = surface_[ct[i]].normal(pts);
