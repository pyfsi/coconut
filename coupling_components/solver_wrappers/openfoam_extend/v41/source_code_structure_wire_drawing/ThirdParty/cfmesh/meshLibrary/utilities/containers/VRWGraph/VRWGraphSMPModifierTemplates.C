...
void VRWGraphSMPModifier::setSizeAndRowSize(const ListType& s)
{
    ...
    # endif
    {
        ...
        # pragma omp single
        {
            procEntries.setSize(omp_get_num_threads());
        }

        sizeType& nLocalEntries = procEntries[omp_get_thread_num()];
        # else
        procEntries.setSize(1);
        sizeType& nLocalEntries = procEntries[0];
        # endif
