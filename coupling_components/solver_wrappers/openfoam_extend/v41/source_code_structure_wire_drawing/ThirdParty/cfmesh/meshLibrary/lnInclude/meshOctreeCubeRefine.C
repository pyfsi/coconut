...
)
{
    ...
    forAllRow(containedElements, containedElementsLabel_, tI)
    {
        ...
        forAllRow(faceEdges, facetI, feI)
        {
            ...
            if( addEdge.find(edgeI) != addEdge.end() )
                continue;
    ...
    if( addEdge.size() != 0 )
    {
        ...
        containedEdges.setSize(containedEdgesLabel_ + 1);
        containedEdges.setRowSize(containedEdgesLabel_, addEdge.size());

        label i(0);
        forAllConstIter(std::set<label>, addEdge, it)
            containedEdges(containedEdgesLabel_, i++) = *it;
    }
}
...
)
{
    ...
    {
        const VRWGraph& containedElmts =
            activeSlotPtr_->containedTriangles_;

        # ifdef DEBUGSearch
        Info << "Distributing contained elements "
            << containedElmts[containedElementsLabel_] << endl;
        # endif
        ...
        # ifdef DEBUGSearch
        boolList transferredElmt
        (
            containedElmts.sizeOfRow(containedElementsLabel_),
            false
        );
        # endif
        ...
        for(label scI=0;scI<4;++scI)
        {
            forAllRow(containedElmts, containedElementsLabel_, tI)
            {
                const label elI =
                    containedElmts(containedElementsLabel_, tI);
        ...
        # ifdef DEBUGSearch
        forAll(transferredElmt, tI)
            if( !transferredElmt[tI] )
                Warning << "Triangle "
                        << containedElmts(containedElementsLabel_, tI)
