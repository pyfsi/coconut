...
:
    ...
    is2DMesh_(false),
    specialMode_(false),
    skipRefinementOfPoorQualityCells_(false),
    maxOrthogonalityDeviationAngle_(70.0),
    minThicknessToFlatnessDeviationHeightRatio_(0.5),
    nLayersAtBndFace_(),
    faceInLayer_(),
    splitEdges_(),
    splitEdgesAtPoint_(),
...
void refineBoundaryLayers::allowToSkipLayerRefinement()
{
    skipRefinementOfPoorQualityCells_ = true;
}

void refineBoundaryLayers::setMaxDeviationAngle(const scalar maxAngleTol)
{
    maxOrthogonalityDeviationAngle_ = maxAngleTol;
}

void refineBoundaryLayers::setMinThicknessToFlatnessDeviationHeight
(
    const scalar minThicknessToFlanessHeight
)
{
    minThicknessToFlatnessDeviationHeightRatio_ = minThicknessToFlanessHeight;
}
...
void refineBoundaryLayers::refineLayers()
{
    ...
    if( !specialMode_ && (globalNumLayers_ < 2) && !refinePatch )
        return;
    ...
    skipRefinementAtBadQualityCells();
...
void refineBoundaryLayers::pointsInBndLayer(labelLongList& layerPoints)
{
    ...
    boolList pointInLayer(mesh_.points().size());
    ...
    # endif
    {
        ...
        # endif
        forAll(pointInLayer, i)
            pointInLayer[i] = false;
    ...
    if( Pstream::parRun() )
    {
        ...
        {
            pointInLayer[globalToLocal[receivedData[i]]] = true;
        }
    }
    ...
    if( meshDict.isDict("boundaryLayers") )
    {
        ...
        if( bndLayers.found("skipRefinementOfPoorQualityLayerCells") )
        {
            ...
            if( skipLayerRefinement )
            {
                ...
                {
                    const scalar minThicknessToFlatnessDeviationRatio =
                        readScalar
                        (
                            bndLayers.lookup
                            (
                    ...
                    refLayers.setMinThicknessToFlatnessDeviationHeight
                    (
                        minThicknessToFlatnessDeviationRatio
                    );
                }
            }
        }
