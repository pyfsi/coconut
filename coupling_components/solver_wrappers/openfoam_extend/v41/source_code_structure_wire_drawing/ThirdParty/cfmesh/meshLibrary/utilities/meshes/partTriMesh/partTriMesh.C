...
:
    ...
    updateBufferLayerAtProcsPtr_(NULL)
{
...
partTriMesh::~partTriMesh()
{
    ...
    deleteDemandDrivenData(updateBufferLayerAtProcsPtr_);
}
...
void partTriMesh::updateVertices(const labelLongList& movedPoints)
{
    ...
    List<direction> updateType(pts.size());

    //- update coordinates of vertices which exist in the surface
    //- of the volume mesh
    std::set<label> bufferPointsSet;
    # ifdef USE_OMP
    # pragma omp parallel
    # endif
    {
        ...
        forAll(movedPoints, i)
        {
            ...
            forAllRow(pointFacets, triPointI, ptI)
            {
                ...
                forAll(tri, j)
                {
                    const label triPointJ = tri[j];

                    if( pointType_[triPointJ] & SMOOTH )
                        updateType[triPointJ] |= SMOOTH;

                    if( pointType_[triPointJ] & FACECENTRE )
                        updateType[triPointJ] |= FACECENTRE;

                    if
                    (
                        updateBufferLayerAtProcsPtr_ &&
                        updateBufferLayerAtProcsPtr_->found(triPointJ)
                    )
                    {
                        # ifdef USE_OMP
                        # pragma omp critical(updateBufferPoints)
                        # endif
                        {
                            bufferPointsSet.insert(triPointJ);
                        }
                    }
                }
            }
        ...
        //- update coordinates of SMOOTH vertices from their coordinates
        //- in the mesh
        # ifdef USE_OMP
        # pragma omp for schedule(dynamic, 50)
        # endif
        forAll(updateType, triPointI)
        {
            if( updateType[triPointI] & SMOOTH )
            {
                const label bpI = pointLabelInMeshSurface_[triPointI];
        ...
        # endif
        {
            forAll(updateType, pI)
            {
                ...
                {
                    # ifdef USE_OMP
                    # pragma omp task shared(pointFacets, pts) firstprivate(pI)
                    # endif
                    {
                        point centre(vector::zero);
                        scalar faceArea(0.0);
    ...
    {
        label i = 0;
        labelLongList bufferPoints(bufferPointsSet.size());
        forAllConstIter(std::set<label>, bufferPointsSet, it)
            bufferPoints[i++] = *it;

        updateBufferLayers(bufferPoints);
    }
}

void partTriMesh::writeToVTK(const fileName& fName) const
{
    ...
    forAll(surf_, tI)
    {
        const labelledTri& t = surf_[tI];

        file << 3 << " " << t[0] << " " << t[1] << " "<< t[2] << nl;
    }

    file << "\nCELL_TYPES " << surf_.size() << nl;
    forAll(surf_, tI)
        file << 5 << nl;
