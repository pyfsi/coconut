...
void polyMeshGenAddressing::calcCellCentresAndVols() const
{
    ...
    cellCentresPtr_ = new vectorLongList(cells.size());
    vectorLongList& cellCtrs = *cellCentresPtr_;

    // Initialise cell volumes to 0
    cellVolumesPtr_ = new scalarLongList(cells.size());
...
(
    const vectorLongList& fCtrs,
    const vectorLongList& fAreas,
    vectorLongList& cellCtrs,
    scalarLongList& cellVols
) const
{
    ...
    # ifdef USE_OMP
    # pragma omp parallel for if( nCells > 1000 ) schedule(guided, 100)
    # endif
    for(label cellI=0;cellI<nCells;++cellI)
    {
        ...
        forAll(c, fI)
        {
            ...
            const vector pc = 0.75 * fCtrs[c[fI]] + 0.25 * cEst;
...
const vectorLongList& polyMeshGenAddressing::cellCentres() const
{
    if( !cellCentresPtr_ )
    {
        # ifdef USE_OMP
...
const scalarLongList& polyMeshGenAddressing::cellVolumes() const
{
    if( !cellVolumesPtr_ )
    {
        # ifdef USE_OMP
