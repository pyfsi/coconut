...
(
    ...
    const scalar gradingFactor,
    const bool symmetric,
    const bool scaleFromZero
)
:
    coordinateModification(),
    origin_(origin),
    ...
    scaleFromZero_(scaleFromZero)
{
    ...
    if( gradingFactor_ < SMALL )
    {
        Warning << "Grading factor for plane " << name << " is smaller than 0.0"
                << endl;
...
void planeScaling::translateAndModifyObject(const vector& disp)
{
    ...
    scalingDistance_ /= (scalingFactor_ + VSMALL);
}
...
vector planeScaling::displacement(const point& p) const
{
    ...
    const vector translationVec = normal_ * scalingDistance_ / scalingFactor_;

    const scalar t = dist / (scalingDistance_ + VSMALL);

    const point pProj = p - dist * normal_;

    if( symmetric_ )
    {
        const scalar tBnd = Foam::max(-1.0, Foam::min(1.0, t));

        if( mag(gradingFactor_ - 1.0) < SMALL )
        {
            //- mesh is not graded
            newP = translationVec * tBnd + pProj;
        }
        else
        {
            //- mesh is graded
            const scalar tQ = (gradingFactor_ - 1.0) * mag(tBnd) + 1.0;
            const scalar tGraded = log(tQ) / log(gradingFactor_);

            newP = translationVec * sign(tBnd) * tGraded + pProj;
        }

        if( t < -1.0 )
            newP -= mag(mag(dist) - scalingDistance_) * normal_;
        if( t > 1.0 )
            newP += (dist - scalingDistance_) * normal_;
    }
    else
    {
        ...
        {
            //- mesh is graded
            const scalar tQ = (gradingFactor_ - 1.0) * tBnd + 1.0;
            const scalar tGraded = log(tQ) / log(gradingFactor_);

            newP = translationVec * tGraded + pProj;
        }
...
vector planeScaling::backwardDisplacement(const point& p) const
{
    ...
    if( symmetric_ )
    {
        ...
        {
            //- mesh is not graded
            newP = tBnd * translationVec + pProj;
        }
        else
        {
            //- mesh is graded
            const scalar tQ = pow(gradingFactor_, mag(tBnd));
            const scalar tGraded = (tQ - 1.0) / (gradingFactor_ - 1.0);
    ...
    else
    {
        ...
        {
            //- mesh is graded
            const scalar tQ = pow(gradingFactor_, tBnd);
            const scalar tGraded = (tQ - 1.0) / (gradingFactor_ - 1.0);
...
void planeScaling::boundingPlanes(PtrList<plane>& pl) const
{
    ...
    {
        pl.set(0, new plane(origin_ - scalingDistance_ * normal_, normal_));
        pl.set(1, new plane(origin_ + scalingDistance_ * normal_, normal_));
    }
}
...
dictionary planeScaling::dict(bool /*ignoreType*/) const
{
    ...
    dict.add("gradingFactor", gradingFactor_);
    dict.add("symmetricScaling", symmetric_);
    dict.add("scaleFromZero", scaleFromZero_);
...
void planeScaling::write(Ostream& os) const
{
    os  << " type:             " << type()
        ...
        << " gradingFactor:    " << gradingFactor_
        << " symmetricScaling: " << symmetric_
        << " scaleFromZero   : " << scaleFromZero_;
}
...
void planeScaling::writeDict(Ostream& os, bool subDict) const
{
    ...
    os.writeKeyword("scalingFactor") << scalingFactor_
                                     << token::END_STATEMENT << nl;
    os.writeKeyword("gradingFactor") << gradingFactor_
                                     << token::END_STATEMENT << nl;
    os.writeKeyword("symmetricScaling") << symmetric_
                                        << token::END_STATEMENT << nl;
    os.writeKeyword("scaleFromZero") << scaleFromZero_
                                     << token::END_STATEMENT << nl;
...
void planeScaling::operator=(const dictionary& d)
{
    ...
    //- specify grading factor
    if( dict.found("gradingFactor") )
    {
        scalar cellSize;
        if( !dict.readIfPresent<scalar>("maxCellSize", cellSize, true) )
        {
            FatalErrorIn
            (
                "void planeScaling::operator=(const dictionary&)"
            ) << "maxCellSize is not available in meshDict!"
              << exit(FatalError);
        }

        const scalar g = readScalar(dict.lookup("gradingFactor"));
        gradingFactor_ =
            exp(log(g)/(1.0 - min(0.1, cellSize / (scalingDistance_+VSMALL))));
    }
    else
    {
        WarningIn
        (
            "void planeScaling::operator=(const dictionary& d)"
        ) << "Entry gradingFactor is not specified!" << endl;

        gradingFactor_ = 1.0;
    }

    //- specify whether the scaling is symmetric or not
    if( dict.found("symmetricScaling") )
    {
        symmetric_ = readBool(dict.lookup("symmetricScaling"));
    }
    else
    {
        symmetric_ = false;
    ...
    //- shall the mesh be graded from the plane towards the interior
    if( dict.found("scaleFromZero") )
    {
        scaleFromZero_ = readBool(dict.lookup("scaleFromZero"));

        if( scaleFromZero_ && (mag(gradingFactor_ - 1.0) > 0.001) )
        {
            scalingFactor_ *=
                (gradingFactor_ - 1.0) /
                (log(gradingFactor_) + VSMALL);

            scalingFactor_ = max(scalingFactor_, 0.001);
        }
    }
    else
    {
