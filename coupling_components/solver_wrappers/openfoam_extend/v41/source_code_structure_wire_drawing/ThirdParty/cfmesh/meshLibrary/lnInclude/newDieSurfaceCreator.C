...
autoPtr<Foam::dieSurfaceCreator>
Foam::dieSurfaceCreator::New
(
...
)
{
    if( debug )
    {
        Info<< "dieSurfaceCreator::New("
            << "const rollingMillPatchNamesHandler& patchNames,"
            << " const dictionary& dict,"
            << " const scalar tol) : "
            << "constructing dieSurfaceCreator"
            << endl;
    }
    ...
    if( cstrIter == dictionaryConstructorTablePtr_->end() )
    {
        ...
        (
            ...
            dict
        )   << "Unknown dieSurfaceCreator type "
            << cmType << nl << nl
            << "Valid dieSurfaceCreator types are :" << nl
            << "[default: " << typeName_() << "]"
