...
void extrudeLayer::createDuplicateFrontFaces(const LongList<labelPair>& front)
{
    ...
    labelLongList faceInFront(faces.size(), -1);
...
{
    pointFieldPMG& points = polyMeshGenModifier(mesh_).pointsAccess();
            ...
            if( it() >= nOrigPoints_ )
            {
                ...
                forAllRow(pointFaces, pointI, pfI)
                {
                    ...
                    if( faceI >= nOrigFaces_ )
                    {
                        ...
                        if( thickness_ < 0.0 )
                        {
                            const vector c = help::faceCentre(points, f);
                            scalar d(VGREAT);
...
void extrudeLayer::updateBoundary()
{
    ...
    const labelLongList& bfOwner = mse.faceOwners();
