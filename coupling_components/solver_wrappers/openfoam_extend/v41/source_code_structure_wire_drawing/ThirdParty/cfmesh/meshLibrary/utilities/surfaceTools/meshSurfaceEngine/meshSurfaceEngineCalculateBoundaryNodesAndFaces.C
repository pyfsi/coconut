...
void meshSurfaceEngine::calculateBoundaryOwners() const
{
    ...
    boundaryFaceOwnersPtr_ = new labelLongList(boundaryFaces.size());

    labelLongList& owners = *boundaryFaceOwnersPtr_;
...
void meshSurfaceEngine::calculateBoundaryNodes() const
{
    ...
    label pointI(0);
    if( !bppPtr_ )
        bppPtr_ = new labelLongList(mesh_.points().size(), -1);
    labelLongList& bp = *bppPtr_;
    ...
    if( Pstream::parRun() )
    {
        ...
        do
        {
            ...
            forAll(procBoundaries, patchI)
            {
                ...
                // PhilipC, use standard labelList for sending and receiving and
                // convert back to longlabelList
                OPstream toOtherProc
                (
                    Pstream::blocking,
                    procBoundaries[patchI].neiProcNo()
            ...
            forAll(procBoundaries, patchI)
            {
                ...
                // PhilipC, use standard labelList for sending and receiving and
                // convert back to longlabelList
                labelList receiveDataLL;
                fromOtherProc >> receiveDataLL;
                forAll(receiveDataLL, i)
                {
                    receiveData.append(receiveDataLL[i]);
                }
    ...
    if( !boundaryPointsPtr_ )
        boundaryPointsPtr_ = new labelLongList();

    labelLongList& boundaryPoints = *boundaryPointsPtr_;
...
void meshSurfaceEngine::calculateBoundaryFacePatches() const
{
    ...
    boundaryFacePatchPtr_ = new labelLongList(bFaces.size());
    labelLongList& facePatch = *boundaryFacePatchPtr_;
...
void meshSurfaceEngine::calculatePointFaces() const
{
    ...
    const labelLongList& bPoints = this->boundaryPoints();
    const faceList::subList& bFaces = this->boundaryFaces();

    //- create boundary points
    const labelLongList& bp = this->bp();
...
void meshSurfaceEngine::calculatePointPatches() const
{
    ...
    const labelLongList& facePatch = boundaryFacePatches();
    const VRWGraph& pFaces = pointFaces();

    labelLongList npPatches(pFaces.size());
    ...
    if( Pstream::parRun() )
    {
        const labelLongList& globalPointLabel = globalBoundaryPointLabel();
...
void meshSurfaceEngine::calculatePointPoints() const
{
    ...
    const labelLongList& boundaryPoints = this->boundaryPoints();
    const faceList::subList& bFaces = this->boundaryFaces();
    const VRWGraph& pFaces = this->pointFaces();
    const labelLongList& bp = this->bp();

    labelLongList npp(boundaryPoints.size());
    ...
    if( Pstream::parRun() )
    {
        ...
        const labelLongList& globalPointLabel = this->globalBoundaryPointLabel();
...
void meshSurfaceEngine::calculatePointNormals() const
{
    ...
    pointNormalsPtr_ = new vectorLongList(pFaces.size());

    meshSurfaceEngineModifier sMod(*this);
    sMod.updateVertexNormals();
}
...
void meshSurfaceEngine::calculateFaceNormals() const
{
    ...
    faceNormalsPtr_ = new vectorLongList(bFaces.size());
...
void meshSurfaceEngine::calculateFaceCentres() const
{
    ...
    faceCentresPtr_ = new vectorLongList(bFaces.size());
    ...
    forAll(bFaces, bfI)
        faceCentresPtr_->operator[](bfI) =
            help::faceCentre(points, bFaces[bfI]);
}
...
void meshSurfaceEngine::calculateEdgesAndAddressing() const
{
    ...
    const labelLongList& bPoints = boundaryPoints();
    const labelLongList& bp = this->bp();

    edgesPtr_ = new edgeLongList();
    edgeLongList& edges = *edgesPtr_;
        ...
        forAll(pFaces, bpI)
        {
            ...
            forAllRow(pFaces, bpI, pfI)
            {
                ...
                if( bp[bf.nextLabel(pos)] >= bpI )
                {
                    ...
                    edgesAtPoint[pp].append(std::make_pair(bfI, false));
                }
                if( bp[bf.prevLabel(pos)] >= bpI )
                {
                    const std::pair<label, label> pp =
                        std::make_pair(bf[pos], bf.prevLabel(pos));
                    edgesAtPoint[pp].append(std::make_pair(bfI, true));
                }
            }
            ...
            {
                const DynList<std::pair<label, bool> >& facesAtEdge =
                    it->second;

                if( facesAtEdge.size() == 2 )
                {
                    //- standard boundary edge with two attached faces
                    if( facesAtEdge[0].first < facesAtEdge[1].first )
                    {
                        if( facesAtEdge[0].second )
                        {
                            //- swap the nodes in the edge
                            edgesHelper.append
                            (
                                edge(it->first.second, it->first.first)
                            );
                        }
                        else
                        {
                            edgesHelper.append
                            (
                                edge(it->first.first, it->first.second)
                            );
                        }
                    }
                    else
                    {
                        if( facesAtEdge[1].second )
                        {
                            //- swap the nodes in the edge
                            edgesHelper.append
                            (
                ...
                else if( facesAtEdge.size() == 1 )
                {
                    //- an inter-processor boundary with a single face
                    if( facesAtEdge[0].second )
                    {
                        //- swap the nodes in the edge
                        edgesHelper.append
                ...
                {
                    //- non-manifold edge or an inter-processor boundary
                    //- with no attached faces
                    label fOwn(bFaces.size()), index(-1);
                    forAll(facesAtEdge, i)
                    {
                        if( facesAtEdge[i].first < fOwn )
                        {
                            fOwn = facesAtEdge[i].first;
                            index = i;
                        }
                    }
    ...
    do
    {
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            // PhilipC, use standard labelList for sending and receiving and
            // convert back to longlabelList
            labelList receivedEdgesLL;
            fromOtherProc >> receivedEdgesLL;
            forAll(receivedEdgesLL, i)
            {
                receivedEdges.append(receivedEdgesLL[i]);
            }
...
void meshSurfaceEngine::calculateEdgePatchesAddressing() const
{
    ...
    const labelLongList& facePatch = this->boundaryFacePatches();
...
void meshSurfaceEngine::calculateFaceFacesAddressing() const
{
    ...
    labelLongList nAppearances(bFaces.size(), 0);
