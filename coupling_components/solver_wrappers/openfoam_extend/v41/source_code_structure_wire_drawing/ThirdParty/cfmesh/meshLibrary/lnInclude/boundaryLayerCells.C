    ...
    forAll(mesh_.boundaries(), patchI)
    {
        if( treatPatches[patchI] )
        {
            const label cId =
                mesh_.addCellSubset
                (
                    "boundaryLayerAtPatch_"+help::labelToText(patchI)
                );
            const label start = mesh_.boundaries()[patchI].patchStart();
            const label size = mesh_.boundaries()[patchI].patchSize();

            for(label fI=0;fI<size;++fI)
            {
                mesh_.addCellToSubset(cId, owner[start+fI]);
            }
        }
    }
    # endif
...
)
{
    ...
    labelLongList otherFaceProc(faceCandidates.size(), -1);
