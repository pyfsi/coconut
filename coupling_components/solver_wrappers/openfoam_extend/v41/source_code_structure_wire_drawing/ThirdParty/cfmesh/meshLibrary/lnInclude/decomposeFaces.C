...
void decomposeFaces::decomposeMeshFaces(const boolList& decomposeFace)
{
    ...
    meshModifier.pointsAccess().setSize(nPoints + nFaces);
    ...
    for(label faceI=0;faceI<nIntFaces;++faceI)
    {
        ...
        if( decomposeFace[faceI] )
        {
            ...
            p = help::faceCentre(points, f);
            meshModifier.movePoint(nPoints, p);
            ++nPoints;
        }
        else
        {
...
)
{
    ...
    for(label faceI=0;faceI<nIntFaces;++faceI)
    {
        ...
        else if( concavePos.size() > 1 )
        {
            ...
            const point fCent = help::faceCentre(points, f);
            mesh_.appendVertex(fCent);
        }
        else
        {
            # ifdef DEBUGDec
