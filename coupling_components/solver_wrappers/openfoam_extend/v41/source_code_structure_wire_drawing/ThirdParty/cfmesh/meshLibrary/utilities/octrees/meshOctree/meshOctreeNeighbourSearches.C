...
) const
{
    ...
    {
        //- point is not inside the octree
        # ifdef OCTREE_DEBUG
...
void meshOctree::findLeavesContainingLine
(
...
) const
{
    ...
    initialCubePtr_->leavesIntersectingLine
    (
        rootBox_,
        startPoint,
        endPoint,
...
) const
{
    ...
    else
    {
        ...
        const label* fNodes =
            meshOctreeCubeCoordinates::neiLeavesOverFace_[dir];
        for(label i=0;i<4;++i)
        {
            if( isQuadtree_ && sc[fNodes[i]] < 0 )
                continue;

            neighbourLeaves.append(sc[fNodes[i]]);
        }
    }
}
