...
(
    ...
    const scalar scaleY,
    const scalar scaleZ,
    const scalar gradingX,
    const scalar gradingY,
    const scalar gradingZ
)
:
    coordinateModification(),
    centre_(centre),
    ...
    gradingVec_(gradingX, gradingY, gradingZ),
    pMin_(),
    pMax_()
{
    calculateBndBox();
...
void boxScaling::translateAndModifyObject(const vector& disp)
{
    ...
    for(direction i=0;i<vector::nComponents;++i)
    {
        ...
        if( mag(gradingVec_[i] - 1.0) < SMALL )
        {
            pMin_[i] = centre_[i] - 0.5 * lengthVec_[i];

        }
        else
        {
            const scalar tQVal = (gradingVec_[i] - 1.0) * 0.5 + 1.0;
            const scalar tGraded = log(tQVal) / log(gradingVec_[i]);

            pMin_[i] = centre_[i] - tGraded * lengthVec_[i];
        }

        pMax_[i] = pMin_[i] + lengthVec_[i];
    }
}

vector boxScaling::displacement(const point& p) const
{
    //- the scaling function if of type f(x) = 1/scaleVec*(x-x0) + x0
    point newP(p);

    for(direction i=0;i<vector::nComponents;++i)
    {
        const scalar scaledLength = lengthVec_[i] / scaleVec_[i];
        ...
        {
            //- mesh is not graded
            newP[i] = scaledLength * tBnd + pMin_[i];
        }
        else
        {
            //- mesh is graded
            const scalar tQ = (gradingVec_[i] - 1.0) * tBnd + 1.0;
            const scalar tGraded = log(tQ) / log(gradingVec_[i]);

            newP[i] = scaledLength * tGraded + pMin_[i];
        }

        newP[i] += min(p[i] - pMin_[i], 0.0);
        newP[i] += max(p[i] - pMax_[i], 0.0);
    }
...
vector boxScaling::backwardDisplacement(const point& p) const
{
    ...
    for(direction i=0;i<vector::nComponents;++i)
    {
        ...
        {
            //- mesh is graded
            const scalar tQ = pow(gradingVec_[i], tBnd);
            const scalar tGraded = (tQ - 1.0) / (gradingVec_[i] - 1.0);
...
dictionary boxScaling::dict(bool /*ignoreType*/) const
{
    ...
    dict.add("gradingX", gradingVec_.x());
    dict.add("gradingY", gradingVec_.y());
    dict.add("gradingZ", gradingVec_.z());
...
void boxScaling::write(Ostream& os) const
{
    os  << " type:      " << type()
        ...
        << " scaleY:    " << scaleVec_.y()
        << " scaleZ:    " << scaleVec_.z()
        << " gradingX:  " << gradingVec_.x()
        << " gradingY:  " << gradingVec_.y()
        << " gradingZ:  " << gradingVec_.z()
        << endl;
}
...
void boxScaling::writeDict(Ostream& os, bool subDict) const
{
    ...
    os.writeKeyword("gradingX") << gradingVec_.x()
         << token::END_STATEMENT << nl;
    os.writeKeyword("gradingY") << gradingVec_.y()
         << token::END_STATEMENT << nl;
    os.writeKeyword("gradingZ") << gradingVec_.z()
         << token::END_STATEMENT << nl;
...
void boxScaling::operator=(const dictionary& d)
{
    ...
    if( !dict.readIfPresent<scalar>("maxCellSize", cellSize, true) )
    {
        FatalErrorIn
        (
            "void boxScaling::operator=(const dictionary&)"
        ) << "maxCellSize is not available in meshDict!"
          << exit(FatalError);
    }

    if( dict.found("gradingX") )
    {
        const scalar g = readScalar(dict.lookup("gradingX"));
        gradingVec_.x() =
            exp(log(g)/(1.0 - min(0.1, cellSize / (lengthVec_.x()+VSMALL))));
    }
    else
    {
        gradingVec_.x() = 1.0;
    ...
    // specify gradingY
    if( dict.found("gradingY") )
    {
        const scalar g = readScalar(dict.lookup("gradingY"));
        gradingVec_.y() =
            exp(log(g)/(1.0 - min(0.1, cellSize / (lengthVec_.y()+VSMALL))));
    }
    else
    {
        gradingVec_.y() = 1.0;
    ...
    // specify gradingZ
    if( dict.found("gradingZ") )
    {
        const scalar g = readScalar(dict.lookup("gradingZ"));
        gradingVec_.z() =
            exp(log(g)/(1.0 - min(0.1, cellSize / (lengthVec_.z()+VSMALL))));
    }
    else
    {
        gradingVec_.z() = 1.0;
