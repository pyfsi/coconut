...
void meshOctreeInsideOutside::frontalMarking()
{
    ...
    # ifdef USE_OMP
    # pragma omp parallel private(frontCubes, neighbours)
    # endif
    {
        ...
        const label chunkSize = max(int(leaves.size()/nThreads), 1);

        const label minLeaf = threadI * chunkSize;
        label maxLeaf = minLeaf + chunkSize;
        if( threadI == (nThreads - 1) )
            maxLeaf = leaves.size();
        maxLeaf = min(maxLeaf, int(leaves.size()));
        ...
        for(label leafI=minLeaf;leafI<maxLeaf;++leafI)
        {
            ...
            label groupI;
            # ifdef USE_OMP
            # pragma omp critical(groupCounter)
            # endif
            groupI = nGroup++;
            ...
            # ifdef USE_OMP
            # pragma omp critical(groupTypeAssignment)
            # endif
            {
        ...
        # ifdef USE_OMP
        # pragma omp critical(neiGroupAssignment)
        # endif
        {
