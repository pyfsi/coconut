...
void meshOctree::findPointsInBox
(
    const boundBox& bb,
    DynList<label>& containedPoints
) const
{
    ...
    forAll(neighbours, neiI)
    {
        ...
        const label ctI = oc.containedElements();
        const VRWGraph& ct = oc.slotPtr()->containedTriangles_;
        forAllRow(ct, ctI, i)
        {
            const labelledTri& tri = surface_[ct(ctI, i)];

            forAll(tri, i)
            {
                const label pI = tri[i];

                if( alreadyAdded.found(pI) )
                    continue;
                alreadyAdded.insert(pI);

                const point p = points[pI];

                if( magSqr(p - c) < dSq )
                    containedPoints.append(pI);
            }
        }
    }
}
