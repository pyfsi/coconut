...
void renameBoundaryPatches::calculateNewBoundary()
{
    ...
    if( dict.found("newPatchNames") )
    {
        ...
        DynList<label, 256> matchedPatches;

        forAll(patchesToRename, patchI)
        {
            if( !patchesToRename[patchI].isDict() )
            {
                Warning << "Cannot rename patch "
                        << patchesToRename[patchI].keyword() << endl;
            ...
            std::map<word, label>::const_iterator patchIt =
                patchToLabel.find(patchName);

            if( patchIt != patchToLabel.end() )
            {
                //- the name exists in the list of patches
                matchedPatches.clear();
                matchedPatches.append(patchIt->second);
            }
            else
            {
                //- the name does not exist in the list of patches
                //- it s probably a regex
                matchedPatches = mesh_.findPatches(patchName);
            }

            if( matchedPatches.size() == 0 )
            {
...
:
    ...
    meshDict_(meshDict)
{
    Info << "Calculating new boundary" << endl;
    if( meshDict.found("renameBoundary") )
        calculateNewBoundary();

    Info << "Checking empty patches" << endl;
    if( !allowEmptyPatches )
    {
        checkEmptyPatches();
    }

    Info << "Udpdating symmetry planes" << endl;
    checkSymmetryPlanes();
}
