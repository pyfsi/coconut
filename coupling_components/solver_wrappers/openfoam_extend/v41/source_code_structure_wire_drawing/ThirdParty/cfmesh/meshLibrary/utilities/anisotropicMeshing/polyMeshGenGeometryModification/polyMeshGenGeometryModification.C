...
void polyMeshGenGeometryModification::modifyGeometry()
{
    ...
    forAll(pts, pointI)
        ...
        (
            pointI,
            coordinateModifierPtr_->modifiedPoint(pts[pointI])
        );
}
...
void polyMeshGenGeometryModification::revertGeometryModification()
{
    ...
    forAll(pts, pointI)
    {
        ...
        if( mesh_.hasBackup(pointI) )
        {
            //- move the backup point to the original space
            //- this may produce invalid mesh in case of severe anisotropy
            point pOrig;
            mesh_.getOrigPoint(pointI, pOrig);

            meshModifier.setBackupPoint
            (
                pointI,
                coordinateModifierPtr_->backwardModifiedPoint(pOrig)
            );
        }
    }
}
