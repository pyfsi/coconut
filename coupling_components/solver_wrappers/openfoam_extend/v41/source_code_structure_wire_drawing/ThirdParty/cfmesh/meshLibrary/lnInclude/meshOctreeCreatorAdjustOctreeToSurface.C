...
void meshOctreeCreator::refineBoxesIntersectingSurfaces()
{
    ...
    boolList refineInside(surfaces.size(), false);
    ...
    forAll(surfaces, surfI)
    {
        ...
        if( dict.found("refineInside") )
            refineInside[surfI] = readBool(dict.lookup("refineInside"));
    ...
    do
    {
        ...
        boolList intersectedLeaf(leaves.size());
        bool useNLayers(false);
        ...
        forAll(surfaceMeshesPtr, surfI)
        {
            ...
            //- check if the octree shall be refined inside the surface
            if( refineInside[surfI] )
            {
                labelList leafInGroup;
                const label nGroups =
                    help::groupMarking
                    (
                        leafInGroup,
                        meshOctreeOps::meshOctreeNeighbourOperator(octree_),
                        meshOctreeOps::meshOctreeNonIntersectedSelectorOperator
                        (
                            intersectedLeaf
                        )
                    );
                ...
                if( nGroups > 1 )
                {
                    labelHashSet matchingGroups;
                    meshOctreeOps::findGroupsByType
                    (
                        octree_,
                        leafInGroup,
                    ...
                    //- mark leaves in the inside groups for refinement
                    # ifdef USE_OMP
                    ...
                    forAll(leafInGroup, leafI)
                    {
                        ...
                        if( matchingGroups.found(leafInGroup[leafI]) )
                        {
                            refineCubes[leafI] |= 1;
                            changed = true;
                        }
