...
void correctEdgesBetweenPatches::decomposeConcaveFaces()
{
    ...
    labelLongList edgeInPatch(edges.size(), -1);
...
void correctEdgesBetweenPatches::patchCorrection()
{
    ...
    const labelLongList& bp = mse.bp();
    const labelLongList& boundaryFaceOwners = mse.faceOwners();
    const labelLongList& facePatches = mse.boundaryFacePatches();
        ...
        forAll(bf, i)
        {
            ...
            )
            {
                if( bf.size() > 4 )
                {
                    ...
                    //- decompose face into quads and a polygon
                    const point p = help::faceCentre(points, bf);

                    DynList<label> newFace;
                    forAll(bf, pI)
                    {
                        newFace.append(points.size());

                        const point pNew = 0.5 * (p + points[bf[pI]]);
                        mesh_.appendVertex(pNew);
                    }

                    //- check fr the existence of point backup
                    if( mesh_.hasPointsBackup() )
                    {
                        pointField backupPoints(bf.size());
                        label pCount(0);

                        forAll(bf, pI)
                        {
                            point pOrig;
                            if( mesh_.getOrigPoint(bf[pI], pOrig) )
                                backupPoints[pCount++] = pOrig;
                        }

                        if( pCount == bf.size() )
                        {
                            DynList<label> copyFace(bf.size());
                            forAll(copyFace, pI)
                                copyFace[pI] = pI;

                            const point origC =
                                help::faceCentre(backupPoints, copyFace);

                            //- create points as if the face was not moved
                            forAll(bf, pI)
                            {
                                const point op =
                                    0.5 * (origC + backupPoints[pI]);

                                polyMeshGenModifier(mesh_).setBackupPoint
                                (
                                    newFace[pI],
                                    op
                                );
                    ...
                    //- append polygon
                    newBoundaryFaces_.appendList(newFace);
                    ...
                    forAll(bf, pI)
                    {
                        ...
                        newF[2] = newFace.fcElement(pI);
                        newF[3] = newFace[pI];

                        newBoundaryFaces_.appendList(newF);
                        newBoundaryOwners_.append(boundaryFaceOwners[bfI]);
                        newBoundaryPatches_.append(facePatches[bfI]);
                    }
