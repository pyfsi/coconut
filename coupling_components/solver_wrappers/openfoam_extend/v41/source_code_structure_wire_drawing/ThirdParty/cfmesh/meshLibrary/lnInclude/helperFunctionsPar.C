...
void whisperReduce(const ListType& neis, const scatterOp& sop, gatherOp& gop)
{
    ...
    forAll(above, aboveI)
    {
        //- receive the data
        List<T> receivedData;
        IPstream fromOtherProc(Pstream::scheduled, above[aboveI]);
    ...
    forAllReverse(below, belowI)
    {
        //- receive the data
        List<T> receivedData;
        IPstream fromOtherProc(Pstream::scheduled, below[belowI]);
        fromOtherProc >> receivedData;
...
)
{
    ...
    labelHashSet receiveData(Pstream::nProcs());
    for(iter=m.begin();iter!=m.end();++iter)
    {
        ...
        if( iter->second.size() > labelMax )
            FatalErrorIn
            (
                "template<class T, class ListType>\n"
                "void exchangeMap("
                "const std::map<label, ListType>&, LongList<T>&,"
                "const Pstream::commsTypes)"
            ) << "Too long data to send" << abort(FatalError);

        toOtherProc << label(iter->second.size());
    }
...
)
{
    ...
    if( commsType == Pstream::scheduled )
    {
        //- start with scheduled data transfer
        //- this type of transfer is intended for long messages because
        //- it does not require any buffer
        ...
        for(riter=m.rbegin();riter!=m.rend();++riter)
        {
            ...
            IPstream fromOtherProc(Pstream::scheduled, riter->first);
            fromOtherProc >> mOut[riter->first];
        }
