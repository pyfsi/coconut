...
void triSurfAddressing::calculatePointNormals() const
{
    ...
    for(label pI=0;pI<size;++pI)
    {
        ...
        std::map<label, vector> patchVectors;

        forAllRow(pFacets, pI, pfI)
            patchVectors[facets_[pFacets(pI, pfI)].region()] = vector::zero;

        if( patchVectors.size() == 1 )
        {
            forAllRow(pFacets, pI, pfI)
            {
                const label tI = pFacets(pI, pfI);
                patchVectors[facets_[tI].region()] += fNormals[tI];
            }

            for
            (
                std::map<label, vector>::iterator it=patchVectors.begin();
                it!=patchVectors.end();
                ++it
            )
            {
...
sizeType triSurfAddressing::printAllocated() const
{
    sizeType totalMemUsage(sizeof(triSurfAddressing));
    ...
    {
        Pout << "pointFacetsPtr "
             << pointFacetsPtr_->printAllocated()/toMb << endl;
        totalMemUsage += pointFacetsPtr_->printAllocated();
    }

    if( edgesPtr_ )
    {
        Pout << "edgesPtr_ " << edgesPtr_->printAllocated()/toMb << endl;
        totalMemUsage += edgesPtr_->printAllocated();
    }

    if( facetEdgesPtr_ )
    {
        Pout << "facetEdgesPtr_ "
             << facetEdgesPtr_->printAllocated()/toMb << endl;
        totalMemUsage += facetEdgesPtr_->printAllocated();
    }

    if( edgeFacetsPtr_ )
    {
        Pout << "edgeFacetsPtr_ "
             << edgeFacetsPtr_->printAllocated()/toMb << endl;
        totalMemUsage += edgeFacetsPtr_->printAllocated();
    }

    if( pointEdgesPtr_ )
    {
        Pout << "pointEdgesPtr_ "
             << pointEdgesPtr_->printAllocated()/toMb << endl;
        totalMemUsage += pointEdgesPtr_->printAllocated();
    }

    if( facetFacetsEdgesPtr_ )
    {
        Pout << "facetFacetsEdgesPtr_ "
             << facetFacetsEdgesPtr_->printAllocated()/toMb
             << endl;
        totalMemUsage += facetFacetsEdgesPtr_->printAllocated();
    }

    if( pointNormalsPtr_ )
    {
        const sizeType usedMem = sizeof(vector) * pointNormalsPtr_->size();
        Pout << "pointNormalsPtr_ " << usedMem/toMb
             << endl;
        totalMemUsage += usedMem;
    }

    if( facetNormalsPtr_ )
    {
        const sizeType usedMem = sizeof(vector) * facetNormalsPtr_->size();
        Pout << " facetNormalsPtr_ " << usedMem/toMb
             << endl;
        totalMemUsage += usedMem;
    }
    ...
    if( facetCentresPtr_ )
    {
        const sizeType usedMem = sizeof(vector) * facetCentresPtr_->size();
        Pout << "facetCentresPtr_ " << usedMem/toMb
             << endl;
        totalMemUsage += usedMem;
    }

    Pout << "triSurfAddressing: Memory usage " << totalMemUsage/toMb << " Mb."
         << endl;
