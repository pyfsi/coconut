...
) const
{
    ...
    copyEdges.setSize(surf_.nFeatureEdges());
    copyEdges = false;
    ...
    //- mark edges contained in selected subsets
    DynList<label> edgeSubsetsIDs;
    surf_.edgeSubsetIndices(edgeSubsetsIDs);

    forAll(edgeSubsetsIDs, i)
    {
        const word esName = surf_.edgeSubsetName(edgeSubsetsIDs[i]);

        forAll(parts, partI)
        {
            if( parts[partI] == esName )
            {
                labelLongList containedEdges;
                surf_.edgesInSubset(edgeSubsetsIDs[i], containedEdges);

                forAll(containedEdges, ceI)
                    copyEdges[containedEdges[ceI]] = true;
    ...
    //- mark all feature edges in selected faces
    const edgeLongList& featureEdges = surf_.featureEdges();
    const VRWGraph& pFacets = surf_.pointFacets();
    ...
    forAll(featureEdges, eI)
    {
        ...
        forAllRow(pFacets, s, ptI)
        {
            const label tI = pFacets(s, ptI);
...
) const
{
    ...
    forAll(newEdgeLabel, edgeI)
    {
        if( copyEdges[edgeI] )
        {
            ...
            if( newPointLabel[e.start()] == -1 )
            {
                newPointLabel[e.start()] = nPoints++;
                edgePoints.append(pts[e.start()]);
            }

            if( newPointLabel[e.end()] == -1 )
            {
                newPointLabel[e.end()] = nPoints++;
                edgePoints.append(pts[e.end()]);
            }
    ...
    if( edgePoints.size() )
    {
        const label nOrigPts = newPts.size();
        newPts.setSize(newPts.size()+edgePoints.size());

        forAll(edgePoints, i)
            newPts[nOrigPts+i] = edgePoints[i];
    }
    ...
    Info << "Copied feature edges " << s.nFeatureEdges() << endl;
    Info << "Copied vertices " << nPoints << endl;
...
void triSurfaceCopyParts::copySurface(const wordList& patches, triSurf& s) const
{
    ...
    markFacetsForCopying(patches, copyFacets, copyEdges);

    copySurfaceMesh(copyFacets, copyEdges, s);
}
...
triSurf* triSurfaceCopyParts::copySurface(const wordList& patches) const
{
    ...
    copySurfaceMesh(copyFacets, copyEdges, *sPtr);
