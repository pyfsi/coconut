...
void cartesianMeshExtractor::createMesh()
{
    ...
    //- fix irregular connections of octree boxes used in the template
    //octreeCheck_.reviseLeaves();
    //octreeCheck_.checkAndFixIrregularConnections();
    ...
    if( mesh_.cells().size() == 0 )
    {
        ...
        << "i.e. 2.49999 instead of 2.5." << endl;

        throw std::string("Terminating meshing process.");
    }
