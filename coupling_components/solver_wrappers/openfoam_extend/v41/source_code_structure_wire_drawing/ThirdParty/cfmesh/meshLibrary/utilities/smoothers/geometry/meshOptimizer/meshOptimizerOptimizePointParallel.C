...
)
{
    ...
    const VRWGraph& pointAtProcs = addressing.pointAtProcs();
    const Map<label>& globalToLocal = addressing.globalToLocalPointAddressing();
    const DynList<label>& neiProcs = addressing.pointNeiProcs();
...
)
{
    ...
    forAll(procPoints, pI)
    {
        ...
        (
            ...
            (
                pointI,
                labelledPointScalar(globalPointLabel[pointI], vector::zero, 0.0)
            )
        );
...
void meshOptimizer::laplaceSmoother::laplacianWHPCParallel
(
    const labelLongList& procPoints
)
{
...
void meshOptimizer::faceFlatnessSmoother::improveFlatnessParallel
(
    const labelLongList& procPoints
)
{
    ...
    const polyMeshGenAddressing& addressing = mesh_.addressingData();
    const VRWGraph& pFaces = mesh_.addressingData().pointFaces();

    const label startProcFace = mesh_.procBoundaries()[0].patchStart();
    ...
    //- allocate space for data that shall be exchanged
    std::map<label, LongList<labelledPointScalar> > exchangeData;
    forAll(pNeiProcs, i)
    ...
    forAll(procPoints, pI)
    {
        ...
        labelledPointScalar& lps = localData[pointI];
        lps = labelledPointScalar(globalPointLabel[pointI], vector::zero, 0.0);

        const point& p = points[pointI];

        forAllRow(pFaces, pointI, pfI)
        {
            const label faceI = pFaces(pointI, pfI);
    ...
    for
    (
        std::map<label, labelledPointScalar>::const_iterator it=
            localData.begin();
        it!=localData.end();
        ++it
    )
    {
        ...
        const labelledPointScalar& lp = it->second;

        const point disp = lp.coordinates() / lp.scalarValue();

        meshModifier.movePoint(pointI, points[pointI] + disp);
    }
