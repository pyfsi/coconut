...
) const
{
    ...
    forAll(neiProcs_, neiProcI)
    {
        ...
        if( toProcs[neiProcI].size() > labelMax )
            FatalErrorIn
            (
                "void meshOctree::exchangeRequestsWithNeighbourProcessors("
                "const LongList<meshOctreeCubeCoordinates>&,"
                "LongList<meshOctreeCubeCoordinates>&) const"
            ) << "The list has too many elements to be sent"
              << abort(FatalError);

        toOtherProc << label(toProcs[neiProcI].size());
    }
    ...
    forAll(neiProcs_, neiProcI)
    {
        ...
        if( toProcs[neiProcI].size() > labelMax )
            ...
            (
                "void meshOctree::exchangeRequestsWithNeighbourProcessors("
                "const LongList<meshOctreeCubeCoordinates>&,"
                " const LongList<scalar>&,"
                " LongList<meshOctreeCubeCoordinates>&,"
                " LongList<scalar>& receivedRanges) const"
