...
{
    labelLongList nonManifoldFacets;

    if( checkForNonManifoldEdges(surf, nonManifoldFacets) )
    {
        ...
        forAll(nonManifoldFacets, i)
            surf.addFacetToSubset(setId, nonManifoldFacets[i]);
    }

    return nonManifoldFacets.size();
}
        ...
        forAll(intersected, tI)
        {
            ...
            forAll(leavesInBox, i)
            {
                ...
                forAll(trianglesInBox, j)
                {
                    ...
                    //- the normal deviation must not be greater than tolerance
                    if( mag(n & normals[triJ]) < cosVal )
                        continue;

                    //- find common points
                    DynList<label> commonPts;
                    for(label pI=0;pI<3;++pI)
                    {
                        for(label pJ=0;pJ<3;++pJ)
                        {
                            if( tri[pI] == nt[pJ] )
                                commonPts.append(tri[pI]);
                        }
                    }
                    ...
                    if( intersect && (commonPolygon.size() > commonPts.size()) )
                    {
                        intersected[tI] = true;
                        intersected[triJ] = true;
