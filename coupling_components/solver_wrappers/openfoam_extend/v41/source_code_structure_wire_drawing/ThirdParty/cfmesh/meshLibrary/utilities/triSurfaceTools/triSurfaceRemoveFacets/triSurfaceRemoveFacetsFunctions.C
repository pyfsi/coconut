...
void triSurfaceRemoveFacets::removeFacets()
{
    ...
    forAll(featureEdges, feI)
    {
        ...
        //- filter out invalid feature edges
        ...
        if
        (
            (e.start() >= newPointLabel.size()) ||
            (e.end() >= newPointLabel.size()) )
            continue;

        //- find global edge label
        const edge newEdge(newPointLabel[e.start()], newPointLabel[e.end()]);

        label eI(-1);
        forAllRow(pointEdges, newEdge.start(), peI)
        {
            const label eJ = pointEdges(newEdge.start(), peI);
