...
void edgeExtractor::calculateSingleCellEdge()
{
    ...
    const labelLongList& faceCells = mse.faceOwners();
...
void edgeExtractor::findPatchesNearSurfaceFace()
{
    ...
    # endif
    {
        ...
        forAll(bFaces, bfI)
        {
            ...
            const vector c = help::faceCentre(points, bf);
...
void edgeExtractor::moveVerticesTowardsDiscontinuities(const label nIterations)
{
    ...
    for(label iterI=0;iterI<nIterations;++iterI)
    {
        ...
        # endif
        {
            ...
            forAll(bFaces, bfI)
            {
                const vector centre = help::faceCentre(points, bFaces[bfI]);
...
void edgeExtractor::distributeBoundaryFaces()
{
    ...
    //- to the face centre
    patchesAtFace_.setSize(bFaces.size());
    bool hasPotentialOverlaps(false);
    ...
    forAll(bFaces, bfI)
    {
        patchesAtFace_[bfI].clear();

        const face& bf = bFaces[bfI];

        const point c = help::faceCentre(points, bf);
        ...
        if( (fPatch > -1) && (fPatch < nPatches) )
        {
            ...
            //- check if there exist patches which can also be candidates
            patchesAtFace_[bfI].insert(fPatch);
            scalar range(0.0);
            forAll(bf, pI)
            {
                range = max(range, mag(c - points[bf[pI]]));
            }
            range = max(range, sqrt(distSq));
            range *= 0.25;

            DynList<label> trias;
            boundBox bb
            (
                c - point(range, range, range),
                c + point(range, range, range)
            );
            meshOctree_.findTrianglesInBox(bb, trias);

            forAll(trias, j)
            {
                const label patchJ = surface[trias[j]].region();

                if( patchJ == fPatch )
                    continue;

                const point np =
                    help::nearestPointOnTheTriangle(trias[j], surface, c);

                if( mag(np - p) < range )
                {
                    patchesAtFace_[bfI].insert(patchJ);
                    hasPotentialOverlaps = true;
                }
            }
        }
        else
    ...
    reduce(hasPotentialOverlaps, maxOp<bool>());

    if( hasPotentialOverlaps )
    {
        //- some boundary faces may be assigned to two or more patches
        //- find the regions where this may happen are find the best patch
        //- for those faces
        std::map<std::set<label>, std::set<label> > combinationToPatches;
        std::map<std::set<label>, std::set<label> >::iterator cIt;
        std::map<label, std::set<std::set<label> > > patchToCombinations;

        //- create a map of combinations and patches assigned to faces
        //- with the same combination
        forAll(patchesAtFace_, bfI)
        {
            std::set<label>& keySet = patchesAtFace_[bfI];

            combinationToPatches[keySet].insert(facePatch_[bfI]);
        }

        if( Pstream::parRun() )
        {
            List<labelLongList> exchangeList(Pstream::nProcs());

            # ifdef DEBUGEdgeExtractor
            forAll(exchangeList, i)
            {
                if( i == Pstream::myProcNo() )
                {
                    for
                    (
                        cIt=combinationToPatches.begin();
                        cIt!=combinationToPatches.end();
                        ++cIt
                    )
                    {
                        const std::set<label>& currKey = cIt->first;
                        const std::set<label>& patches = cIt->second;

                        DynList<label> keys, patchesCopy;
                        forAllConstIter(std::set<label>, currKey, it)
                            keys.append(*it);
                        forAllConstIter(std::set<label>, patches, it)
                            patchesCopy.append(*it);

                        Pout << "1.Combination " << keys << " patches "
                             << patchesCopy << endl;
                    }
                }
            ...
            //- prepare data that shall be exchanged with other processes
            labelLongList& currList = exchangeList[Pstream::myProcNo()];
            for
            (
                cIt=combinationToPatches.begin();
                cIt!=combinationToPatches.end();
            ...
            )
            {
                ...
                //- flatten the map into a label list
                //- send keys
                currList.append(currKey.size());
                forAllConstIter(std::set<label>, currKey, it)
                    currList.append(*it);

                //- send patches for a given combination
                currList.append(patches.size());
                forAllConstIter(std::set<label>, patches, it)
                    currList.append(*it);
            }

            //- exchange data among processors
            Pstream::gatherList(exchangeList);
            Pstream::scatterList(exchangeList);

            forAll(exchangeList, procI)
            {
                if( procI == Pstream::myProcNo() )
                    continue;

                const labelLongList& receivedData = exchangeList[procI];

                label counter(0);

                while( counter < receivedData.size() )
                {
                    const label nInCombination = receivedData[counter++];

                    std::set<label> keySet;
                    for(label i=0;i<nInCombination;++i)
                        keySet.insert(receivedData[counter++]);

                    std::set<label>& patches = combinationToPatches[keySet];

                    const label nPatches = receivedData[counter++];
                    for(label i=0;i<nPatches;++i)
                        patches.insert(receivedData[counter++]);
                }
            }
            ...
            forAll(exchangeList, i)
            {
                if( i == Pstream::myProcNo() )
                {
                    ...
                    )
                    {
                        ...
                        Pout << "2.Combination " << keys << " patches "
                             << patchesCopy << endl;
                    }
                }
        ...
        //- create a map of combinations for each patch
        for
        (
            cIt=combinationToPatches.begin();
            cIt!=combinationToPatches.end();
        ...
        {
            const std::set<label>& keySet = cIt->first;

            forAllConstIter(std::set<label>, keySet, pIt)
                patchToCombinations[*pIt].insert(keySet);
        }

        //- determine a patch for combinations with two or more patches
        for
        (
            cIt=combinationToPatches.begin();
            cIt!=combinationToPatches.end();
        ...
        {
            std::set<label>& patches = cIt->second;

            //- combination where all faces are in the same patch
            //- such combinations do not have to be procesed further
            if( patches.size() == 1 )
                continue;

            //- check if there exists a combination consisting of a single
            //- patch that is also part of the current combination
            label uniquePatch(-1);
            forAllConstIter(std::set<label>, patches, pIt)
            {
                //- generate a set with this id and find out whether it exists
                //- in the combinations map
                std::set<label> currKey;
                currKey.insert(*pIt);

                if
                (
                    combinationToPatches.find(currKey) !=
                    combinationToPatches.end()
                )
                {
                    uniquePatch = *pIt;
                    break;
            ...
            if( uniquePatch != -1 )
            {
                //- all faces with this combination shall be assigned
                //- to the same patch
                patches.clear();
                patches.insert(uniquePatch);
            }
            else
            {
                //- none of the patches has a face with candidates
                //- in that patch, only.
                //- choose a patch in the smallest number of combinations
                label nCombinations(labelMax);
                uniquePatch = -1;

                std::map<label, std::set<std::set<label> > >::const_iterator i;
                for
                (
                    i=patchToCombinations.begin();
                    i!=patchToCombinations.end();
                    ++i
                )
                {
                    const label nCurrCombinations = i->second.size();

                    if( nCurrCombinations < nCombinations )
                    {
                        uniquePatch = i->first;
                        nCombinations = nCurrCombinations;
                    }
                }

                //- assing all faces in this combination into the same patch
                patches.clear();
                patches.insert(uniquePatch);
            }
        }
        ...
        # ifdef USE_OMP
        # pragma omp parallel for schedule(dynamic, 50) private(cIt)
        # endif
        forAll(facePatch_, bfI)
        {
            const std::set<label>& combination = patchesAtFace_[bfI];

            cIt = combinationToPatches.find(combination);

            if( cIt != combinationToPatches.end() )
            {
                const std::set<label>& patches = cIt->second;
                ...
                if( patches.size() > 1 )
                    FatalErrorIn
                    (
                        "edgeExtractor::distributeBoundaryFaces()"
                    ) << "More than one patch" << abort(FatalError);

                facePatch_[bfI] = *patches.begin();
            }
            else
            {
                FatalErrorIn
                (
                    "edgeExtractor::distributeBoundaryFaces()"
                ) << "Combination not found " << abort(FatalError);
            }
        }
    }
}
...
bool edgeExtractor::distributeBoundaryFacesNormalAlignment()
{
    ...
    do
    {
        ...
        forAll(candidates, i)
        {
            ...
            const std::set<label>& patchesInOverlap = patchesAtFace_[bfI];
            ...
            forAll(allNeiPatches, i)
            {
                ...
                point p = help::faceCentre(points, bf);
            ...
            (
                (newPatch >= 0) &&
                (newPatch != facePatch_[bfI]) &&
                (patchesInOverlap.find(newPatch) == patchesInOverlap.end())
            )
            {
...
bool edgeExtractor::checkFacePatchesTopology()
{
    ...
    do
    {
        ...
        forAll(candidates, i)
        {
            ...
            //- do not swap in case the patch is negative or the same patch
            if( (newPatch < 0) || (newPatch == facePatch_[bfI]) )
                continue;

            //- check whether the edges shared with the neighbour patch form
        ...
        //- evaluate the new situation and ensure that no oscillations occur
        reduce(nCorrected, sumOp<label>());
        if( nCorrected )
        {
...
bool edgeExtractor::checkFacePatchesGeometry()
{
    ...
    //- allocate a copy of boundary patches
    labelLongList newBoundaryPatches(facePatch_.size());
    ...
    do
    {
        ...
        Info << "Projecting onto feature edges" << endl;
        ...
        meshSurfaceEngineModifier(mse).updateGeometry(activePointLabel);
        ...
        Info << "Found " << invertedPoints.size()
        ...
        mso.untangleAndRevertSurface(activePointLabel, 1);
...
{
    notImplemented("void edgeExtractor::projectDeterminedFeatureVertices()");
...
{
    notImplemented("bool edgeExtractor::untangleSurface(");
    ...
    meshSurfaceOptimizer optimizer(mse);
...
//    {
//        Info << "Finished topological adjustment of concave edges" << endl;

//        # ifdef DEBUGEdgeExtractor
//        Info << "Changes due to concave edges" << endl;
//        fileName sName("checkConcaveEdges"+help::scalarToText(nIter)+".stl");
//        sPtr = surfaceWithPatches();
//        sPtr->writeSurface(sName);
//        deleteDemandDrivenData(sPtr);
//        # endif
...
//    else
//    {
//        Info << "No changes at concave edges were needed" << endl;
//    }
