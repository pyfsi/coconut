...
void boundaryLayers::findPatchesToBeTreatedTogether()
{
    ...
    const labelLongList& boundaryFacePatches = mse.boundaryFacePatches();
    ...
    forAllConstIter(labelHashSet, corners, it)
    {
        ...
        if( mPart.numberOfFeatureEdgesAtPoint(bpI) > 3 )
        {
            ...
            forAllRow(pPatches, bpI, patchI)
            {
                patchAtValenceCorner_[pPatches(bpI, patchI)] = true;
    ...
    if( Pstream::parRun() )
    {
        ...
        const labelLongList& globalEdgeLabel = mse.globalBoundaryEdgeLabel();
            ...
            forAll(f, pI)
            {
                ...
                else
                {
                    ...
                    if( !transferredPointToLocal.found(lp.pointLabel()) )
                    {
                        ...
                        mesh_.appendVertex(lp.coordinates());
                    }
        ...
        polyMeshGenModifier(mesh_).pointsAccess().setSize(nPoints_);
    }
    ...
    //- update the information with the user settings
    updatePatchesToBeTreatedTogether();
...
void boundaryLayers::updatePatchesToBeTreatedTogether()
{
    if( !mesh_.returnTime().foundObject<dictionary>("meshDict") )
        return;
    ...
    //- the user can override the default layer topology for inlet and outlet
    //- patches
    if( bndLayers.isDict("inletOutletPatches") )
    {
        ...
        forAll(patchNames, i)
        {
            //- find patch index
            label patchI(-1);
            for(patchI=0;patchI<patchNames_.size();++patchI)
                if( patchNames_[patchI] == patchNames[i] )
                    break;

            if( patchI >= patchNames_.size() )
            {
                WarningIn
                (
                    "void boundaryLayers::updatePatchesToBeTreatedTogether()"
                ) << "Patch " << patchNames[i] << " does not exist "
                  << "as a patch name " << patchNames_ << endl;
                continue;
            }

            const dictionary& pDict = inletOutletDict.subDict(patchNames[i]);
            bool generateLayer(true);
            if( pDict.found("generateLayer") )
                generateLayer = readBool(pDict.lookup("generateLayer"));

            if( (treatPatchesWithPatch_[patchI].size() > 1) || !generateLayer )
            {
                //- disconnect the layer at this patch from layers in other
                //- neighbour patches
                DynList<label>& neiPatches = treatPatchesWithPatch_[patchI];

                # ifdef USE_OMP
                # pragma omp parallel for schedule(dynamic, 50) \
                if( treatedPatch_.size() > 1000)
                # endif
                forAll(treatPatchesWithPatch_, patchJ)
                {
                    if( patchI == patchJ )
                        continue;

                    DynList<label>& oNeiPatches =
                        treatPatchesWithPatch_[patchJ];

                    const label pos = oNeiPatches.containsAtPosition(patchI);

                    if( pos < 0 )
                        continue;

                    oNeiPatches.removeElement(pos);
                }

                neiPatches.setSize(1);
                neiPatches = patchI;

                if( patchAtValenceCorner_[patchI] )
                {
                    WarningIn
                    (
                        "void boundaryLayers::"
                        "updatePatchesToBeTreatedTogether()"
                    ) << "It is not possible to generate boundary layer "
                      << "at patch "
                      << patchNames_[patchI] << endl;

                    treatedPatch_[patchI] = true;
                }
                else if( !generateLayer )
                {
                    treatedPatch_[patchI] = true;
                }
            }
    ...
    //- the user can specify a global setting for symmetry plane ensuring
    //- that boundary layers exit out at the symmetry plane
        ...
        if( symmPlaneLayerTopo )
        {
            ...
            if( bndLayers.found("generateLayersAtSymmetryPlanes") )
                generateLayersAtSymmetryPlanes =
                    readBool
                    (
                        bndLayers.lookup("generateLayersAtSymmetryPlanes")
                    );
            ...
            forAll(patchTypes_, patchI)
            {
                if
                (
                    (patchTypes_[patchI] == "symmetry") ||
                    (patchTypes_[patchI] == "symmetryPlane")
                )
                {
                    //- disconnect the layer at this patch from layers in other
                    //- neighbour patches
                    ...
                    forAll(treatPatchesWithPatch_, patchJ)
                    {
                        ...
                        const label pos =
                            oNeiPatches.containsAtPosition(patchI);
                    ...
                    //- check if it is possible to get decent
                    ...
                    else if( !generateLayersAtSymmetryPlanes )
                    {
                        treatedPatch_[patchI] = true;
                    }
                }
    ...
    geometryAnalysed_(false)
{
    ...
    patchAtValenceCorner_.setSize(boundaries.size());
    patchAtValenceCorner_ = false;
...
void boundaryLayers::addLayerForPatch(const word& patchName)
{
    ...
    wordHashSet layerAtPatches;
    if( mesh_.metaData().found("boundaryLayerAtPatches") )
        layerAtPatches =
            wordHashSet(mesh_.metaData().lookup("boundaryLayerAtPatches"));

    layerAtPatches.insert(patchName);
    mesh_.metaData().add("boundaryLayerAtPatches", layerAtPatches, true);
}
...
void boundaryLayers::activate2DMode()
{
    ...
    const labelLongList& facePatch = surfaceEngine().boundaryFacePatches();
...
void boundaryLayers::addLayerForAllPatches()
{
    ...
    {
        if( !treatedPatch_[patchI] )
            layerAtPatches.insert(boundaries[patchI].patchName());
    }
