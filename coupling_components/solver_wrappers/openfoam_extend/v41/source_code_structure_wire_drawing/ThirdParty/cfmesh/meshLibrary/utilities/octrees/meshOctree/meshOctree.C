...
(
    const triSurf& ts,
    const DynList<label>& activeTriangles,
    const boundBox& rootBox,
    const bool isQuadtree
)
:
    surface_(ts),
    neiProcs_(),
    ...
    isQuadtree_(isQuadtree)
{
    createInitialOctreeBox(&rootBox, &activeTriangles);
...
(
    const boundBox* bbPtr,
    const DynList<label>* containedTrianglesPtr
)
{
    //- create initial octree box
    if( bbPtr )
    {
        const point& min_ = bbPtr->min();
        const point& max_ = bbPtr->max();
...
sizeType meshOctree::printAllocated() const
{
    sizeType totalMemUsage(sizeof(meshOctree));

    totalMemUsage += neiProcs_.byteSize();

    totalMemUsage += sizeof(Pair<meshOctreeCubeCoordinates>) * neiRange_.size();

    forAll(dataSlots_, i)
    {
        totalMemUsage += sizeof(meshOctreeCube) * dataSlots_[i].cubes_.size();
        totalMemUsage += dataSlots_[i].childCubes_.printAllocated();
        totalMemUsage += dataSlots_[i].containedTriangles_.printAllocated();
        totalMemUsage += dataSlots_[i].containedEdges_.printAllocated();
    }

    totalMemUsage += sizeof(meshOctreeCube*) * leaves_.size();

    Pout << "meshOctree: memory usage "
         << (totalMemUsage/1048576) << " Mb of memory" << endl;
