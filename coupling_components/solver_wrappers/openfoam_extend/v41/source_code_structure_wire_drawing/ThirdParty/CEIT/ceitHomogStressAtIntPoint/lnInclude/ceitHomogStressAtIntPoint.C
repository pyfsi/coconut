...
#include "ceitHomogStressAtIntPoint.H"
...
namespace Foam
{
    defineTypeNameAndDebug(ceitHomogStressAtIntPoint, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, ceitHomogStressAtIntPoint, dictionary
    );
    ...
    extern "C"
    {
        ...
        // uppercase letters
        void homogstressatintpoint_v5_
        (
            const double Fnew[9],
            const double Fold[9],
            const double props[11],
            const double statOld[247],
            double statNew[247],
            double sNew[6]
        );
    }
...
void Foam::ceitHomogStressAtIntPoint::calculateStressAtPoint
(
    const tensor& Fnew,
    const tensor& Fold,
...
)
{
  ...
  // need to reorder to column major indexing for fortran
    ...
    // Info << "fortranFnew: " << Fnew << endl;
    // Info << "fortranFold: " << Fold << endl;
    // //Info << "fortranProps: " << fortranProps << endl;
    // Info << "fortranStatOld: " << endl;
    // for (int i=4; i<13; i++)
    //     Info << fortranStatOld[i] << " ";
    // Info << endl;
    // //Info << "fortranStatNew: " << fortranStatNew << endl;
    // //Info << "fortranSNew: " << fortranSNew << endl;

    
    homogstressatintpoint_v5_
    (
        fortranFnew,
        fortranFold,
        fortranProps,
...
Foam::ceitHomogStressAtIntPoint::ceitHomogStressAtIntPoint
(
    const word& name,
    const fvMesh& mesh,
...
:
    ...
    lamellarSpacing_(readScalar(dict.lookup("lamellarSpacing"))),
    seed_(readLabel(dict.lookup("seed"))),    
    stats_(0),
    U_
    (
        IOobject
...
{
    // NOTE: Currently initialising ALL homogenised material points
    // to the same initial random vector direction
    // Unixial tensile test has direction init to 0,0,1

    Random R(seed_);
    vector vr((R.scalar01()-1.0)*2.0,(R.scalar01()-1.0)*2.0,(R.scalar01()-1.0)*2.0);
    vr = vr/mag(vr);
    ...
    {
        scalar stateVariableValue = 0;
        if(fieldI == 0)
        {
            //stateVariableValue = vr.x();
        }
        else if(fieldI == 1)
        {
            //stateVariableValue = vr.y();
        }
        else if(fieldI == 2)
        {
            //stateVariableValue = vr.z();
            stateVariableValue = 1.0;
        }
        else if(fieldI == 4)
        {
            stateVariableValue = 1.0;
        ...
        {
            stateVariableValue = lamellarSpacing_;
        }
        ...
        (
            ...
            (
                ...
                mesh,
                dimensionedScalar("zero", dimless, stateVariableValue)
            )
        );
    ...
    // {
    //     WarningIn("ceitHomogStressAtIntPoint::ceitHomogStressAtIntPoint()")
...
Foam::tmp<Foam::volScalarField> Foam::ceitHomogStressAtIntPoint::rho() const
{
...
Foam::tmp<Foam::volScalarField> Foam::ceitHomogStressAtIntPoint::E() const
{
    return tmp<volScalarField>
    (
        new volScalarField
        (
            ...
            mesh(),
            dimensionedScalar("E", dimPressure, props_[0]),
...
Foam::tmp<Foam::volScalarField> Foam::ceitHomogStressAtIntPoint::nu() const
{
    return tmp<volScalarField>
    (
        new volScalarField
        (
            ...
            mesh(),
            dimensionedScalar("nu", dimless, props_[1]),
            zeroGradientFvPatchScalarField::typeName
        )
    );
}
...
Foam::ceitHomogStressAtIntPoint::RhieChowScaleFactor() const
{
...
void Foam::ceitHomogStressAtIntPoint::correct(volSymmTensorField& tau)
{
    ...
    forAll(sNewI, cellI)
    {
	  ...
	  statOldCellI[fieldI] = stats_[fieldI].oldTime()[cellI];
        ...
        if (curMaterial().internalField()[cellI] == 1)
            {
    ...
    forAll(tau.boundaryField(), patchI)
    {
        ...
        forAll(sNewP, faceI)
        {
            ...
            if (curMaterial().boundaryField()[patchI][faceI] == 1)
                {
                    calculateStressAtPoint
                        (
                         FnewP[faceI],
...
void Foam::ceitHomogStressAtIntPoint::correct
(
...
void Foam::ceitHomogStressAtIntPoint::correct(surfaceSymmTensorField& tau)
{
    notImplemented
    (
        "ceitHomogStressAtIntPoint::correct(surfaceSymmTensorField& tau)"
    );
}


Foam::scalar Foam::ceitHomogStressAtIntPoint::residual()
{
...
void Foam::ceitHomogStressAtIntPoint::updateYieldStress()
{
...
Foam::tmp<Foam::volScalarField>
Foam::ceitHomogStressAtIntPoint::plasticDissipationRate() const
{
...
Foam::scalar Foam::ceitHomogStressAtIntPoint::newDeltaT() const
{
