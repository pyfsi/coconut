...
def create(parameters):
    return SolverWrapperOpenFoamESI(parameters)
...
class SolverWrapperOpenFoamESI(SolverWrapper):
    ...
    def check_software(self):
        ...
        # check version
        if version_nr.find(self.version)==-1:
            raise RuntimeError(
    ...
    def read_modify_controldict(self):
        """
        reads the controlDict file in the case-directory and modifies some entries required by the wrapper's solver.
    ...
    def write_of_residuals(self):
        """
        reads the log file generated by the wrapper's solver and writes the last initial residual of the fields
