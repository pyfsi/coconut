...
from coconut.data_structure.variables import accepted_variables_pc_solid
...
def create(parameters):
    return SolverWrapperPCFluentSolid(parameters)


class SolverWrapperPCFluentSolid(SolverWrapper):
    ...
    accepted_in_var = list(accepted_variables_pc_solid['in'].keys())
    accepted_out_var = list(accepted_variables_pc_solid['out'].keys())
        ...
        for var in self.input_variables:
            ...
            if self.debug:
                for dct in self.interface_input.parameters:
                    ...
                    thread_id = self.model_part_thread_ids[mp_name]
                    src = accepted_variables_pc_solid['in'][var][0] + f'_timestep{self.timestep}_thread{thread_id}.dat'
                    dst = accepted_variables_pc_solid['in'][var][0] + f'_timestep{self.timestep}_thread{thread_id}_Iter{self.iteration}.dat'
            ...
            for var in dct['variables']:
                ...
                if prefix != 'skip':
                    ...
                    if accepted_variables_pc_solid['out'][var][1] == 0:
                        ...
                        req_dim = min([self.dimensions, accepted_variables_pc_solid['out'][var][1]]) + self.mnpf
                    ...
                    if accepted_variables_pc_solid['out'][var][1] == 1:
                        # get face coordinates and ids
    ...
    def remove_dat_files(self, timestep):
        if not self.debug:
            for thread_id in self.thread_ids.values():
                for var in self.output_variables:
                    if accepted_variables_pc_solid['out'][var][0] != 'skip':
                        try:
                            os.remove(join(self.dir_cfd, accepted_variables_pc_solid['out'][var][0] + f'_timestep{timestep}_thread{thread_id}.dat'))
                        except OSError:
                            pass
                    if var == 'displacement':
                        # solid zone outputs displacement and creates input file for itself at the same time
                        try:
                            os.remove(join(self.dir_cfd, accepted_variables_pc_solid['in'][var][0] + f'_timestep{timestep}_thread{thread_id}.dat'))
                        except OSError:
                            pass
                for var in self.input_variables:
                    try:
            ...
            for dct in self.interface_input.parameters:
                ...
                if 'faces' in mp_name:
                    ...
                    for i in range(self.mnpf):
                        fmt += '%27d'
                    prefix = accepted_variables_pc_solid['in'][var][0]
