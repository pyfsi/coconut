...
def create(parameters):
    return SolverWrapperAbaqusLineLoad(parameters)


class SolverWrapperAbaqusLineLoad(SolverWrapper):
    ...
    def initialize(self):
        ...
        if self.timestep_start == 0:  # run USRInit only for new calculation
            usr = 'USRInit_line_load.f'
            with open(join(self.path_src, usr), 'r') as infile:
                with open(join(self.dir_csm, 'usrInit_line_load.f'), 'w') as outfile:
                    for line in infile:
            ...
            cmd = f'abaqus make library=usrInit_line_load.f directory={path_libusr} >> {self.logfile} 2>&1'
            self.print_log(f'### Compilation of usrInit_line_load.f ###')
            ...
            self.print_log(f'\n### Get load integration points using usrInit_line_load.f ###')
        ...
        usr = 'USR_line_load.f'
        with open(join(self.path_src, usr), 'r') as infile:
            with open(join(self.dir_csm, 'usr_line_load.f'), 'w') as outfile:
                for line in infile:
                    line = line.replace('|dimension|', str(self.dimensions))
                    ...
                    if '|' in line:
                        raise ValueError(
                            f'The following line in USR_line_load.f still contains a \'|\' after substitution: \n '
                            f'\t{line} \n Probably a parameter was not substituted')
                    outfile.write(line)

        # compile Abaqus USR.f
        self.print_log(f'\n### Compilation of usr_line_load.f ###')
        shutil.rmtree(path_libusr)  # remove libusr containing compiled USRInit_line_load.f
        os.mkdir(path_libusr)
        cmd = f'abaqus make library=usr_line_load.f directory={path_libusr} >> {self.logfile} 2>&1'
        subprocess.run(cmd, shell=True, cwd=self.dir_csm, executable='/bin/bash', env=self.env)
    ...
    def write_loads(self):
        ...
        for mp_id, item in enumerate(self.settings['interface_input']):
            ...
            data = traction[:, :self.dimensions]
            fmt = self.dimensions * '%27.17e'  # format of load input file should be exactly this for USR.f
