...
class MapperWedge3DToAxisymmetric2D(Component):
    def __init__(self, parameters):
        ...
        self.points = self.settings['points'].GetBool()
        ...
        if forward:
            ...
            if self.points == True:
                ...
                for i_from,node in enumerate(model_part_in.Nodes):
                    ...
                    self.nearest[i_to] = i_from
                    r = coords[self.dir_r]
                    coords[self.dir_r] = r / np.cos(self.angle)
                    coords[self.dir_wedge] = 0
        ...
        # vector interpolation
            ...
            if self.points == True:
                for i_to, node_to in enumerate(model_part_to.Nodes):
                    ...
                    hist_var_to[self.dir_r] = tmp[self.dir_r] * np.cos(-self.angle) + tmp[self.dir_wedge] * np.cos(
                        np.pi / 2 - self.angle)
                ...
                for i_to, node_to in enumerate(model_part_to.Nodes):
                    ...
                    hist_var_to[self.dir_a] = tmp[self.dir_a]
                    hist_var_to[self.dir_r] = tmp[self.dir_r] / np.cos(self.angle)
                    # print("hist_to")
                    # print(hist_var_to[self.dir_r])
