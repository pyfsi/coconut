...
class MapperLinearBounded(Component):
    ...
    def initialize(self, model_part_from, model_part_to):
        ...
        _, self.nearest = self.tree.query(self.coords_to, k=self.n_nearest)
    ...
    def __call__(self, args_from, args_to):
        ...
        # add z-coordinate to data_to
        if np.shape(data_to)[1] == 2:
            data_to = np.append(data_to, np.zeros((np.shape(data_to)[0], 1)), axis=1)
