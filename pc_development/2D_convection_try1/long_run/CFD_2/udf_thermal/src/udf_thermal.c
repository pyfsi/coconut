...
DEFINE_GRID_MOTION(move_nodes, domain, dynamic_thread, time, dtime) {
    ...
    if (stat("/tmp/coconut_victor_3707546_fluent", &st) == -1) {
        mkdir("/tmp/coconut_victor_3707546_fluent", 0700);
    }
    sprintf(file_name, "/tmp/coconut_victor_3707546_fluent/nodes_update_timestep%i_thread%i.dat",
            timestep, thread_id);
    host_to_node_sync_file("/tmp/coconut_victor_3707546_fluent");  /* receive file on compute nodes and store in a temporary folder */
#endif /* RP_HOST */
