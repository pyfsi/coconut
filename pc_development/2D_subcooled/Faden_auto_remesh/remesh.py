...
def run_cfd_workflow(switch_run_num):
    ...
    remesh_files_1 = ['remesh_quad.sh', 'remesh_quad.jou', 'remesh_tri.sh', 'remesh_tri.jou']
    remesh_files_2 = ['remesh.sh', 'remesh.jou']

    try:
        for f in remesh_files_1:
            shutil.copy2(os.path.join(setup_files_solid_remesh_dir, f), cfd1_remesh_dir)
            print(f"  Copied '{f}' to '{cfd1_remesh_dir}'")
    except FileNotFoundError as e:
        print(f"  Error: {e}. Make sure 'remesh_quad/tri.sh' and 'remesh_quad/tri.jou' exist in 'setup_files/solid/create_mesh/'.")
        return
    except Exception as e:
